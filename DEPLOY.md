# Deploying a Lesto app

Lesto's deployment model is deliberately small. There is **one durable
...
A Lesto app is a single `lesto.app.ts` that default-exports its `AppConfig`
(`{ db, router, controllers, migrations }`). Two kinds of process run it, plus
one one-shot command:

| Process            | Command            | Role                                                        |
| ------------------ | ------------------ | ----------------------------------------------------------- |
| Web tier           | `lesto serve`       | Boots the HTTP server in front of the app. **Stateless.**   |
| Worker tier        | `lesto work` *      | Drains the durable job queue. **Stateless.**                |
| Migrations         | `lesto migrate`     | Applies pending migrations, prints the versions, exits.     |

\* The worker is `runWorker(queue)` from `@lesto/runtime` — a thin wire over
...
`lesto serve` boots the web tier only, so the two scale independently.

## The web tier is stateless

`lesto serve` holds nothing in memory that matters across requests. Every request
is resolved from the database. That means:

- **Scale horizontally** by running more `lesto serve` containers behind a load
...
`--production` dependency install, the app source, and `lesto serve --port $PORT`.

## The database is the one substrate

Lesto puts the job queue **on the SQL database** (`@lesto/queue`) rather than on a
...
- **Migrations run against this one substrate.** Run `lesto migrate` once per
...
Out of the box a Lesto app uses an embedded SQLite database (`lesto.db`), which is
perfect for a single node and for getting to production fast. To scale past one
box, point the app at a managed datastore:

- **Postgres** — supply a Postgres-backed `db` in `lesto.app.ts`. The queue,
...
- **Redis adapters** — for the hot paths that want an in-memory tier
  (`@lesto/cache`, `@lesto/ratelimit`, `@lesto/pubsub`), swap the in-memory store
...
1. **Build** the image: `docker build -t my-lesto-app .`
2. **Migrate** against the target database: `lesto migrate` (one-shot, pre-deploy).
3. **Roll the web tier**: deploy the new `lesto serve` instances behind the LB.
