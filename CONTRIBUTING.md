...
Thanks for your interest in Keel. This document is the practical companion to
...
- **Node ≥ 22** — Keel targets Node 22+ for runtime compatibility.
...
- **100% vitest coverage** — lines, branches, functions, statements — on every
  ...
  have written.
- **Errors carry codes.** Every failure is a `KeelError` subclass with a stable,
...
```sh
bun run ws:typecheck       # strict tsc across every @keel/* package
...
```sh
bun run --filter '@keel/*' format   # writes oxfmt's formatting
```
...
- **Features** — open an issue with the feature template first so we can align on
  the design before code. Keel is opinionated and batteries-included; a feature
