 ...
 * `lesto.sites.ts` and the `lesto.build.ts` hook), and the edge serves those files directly.
 *
...
export async function buildAppConfig(): Promise<LestoAppConfig> {
  ...
  // (server) and the deferred search island (client) on Preact — the ~10 KB runtime,
  // and a single dialect so `lesto build` never diverges them. `css` is the Tailwind
  // entry; `cssScanRoot: "src"` points Tailwind at the docs chrome's components (they
  // live in `src/`, not the default `app/`), so their utility classes are compiled in.
  return {
    db,
    app,
    migrations: "skip",
