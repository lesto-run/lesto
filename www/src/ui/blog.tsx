 ...
 * `/blog/<slug>`.
 *
 * Both reuse the global `SiteLayout` chrome (header, footer) and the `.prose`
 * typography for rendered Markdown — they differ only in the centered,
 * single-column `.prose-shell` frame. The `make*` helpers close their data into
 * a zero-prop component so the app can register one static `.page()` per route
 * with no loader.
 */
