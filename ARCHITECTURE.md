# Volo — Architecture & Product Vision
...
>
> **Honesty note (v1):** the extensibility model is a *designed, deferred* bet, not a shipped battery — see [ADR 0014](./docs/adr/0014-plugin-system.md). The earlier `@volo/hooks`/`@volo/config` prototypes were orphans (zero importers) and were removed from the v1 surface; §3.5, §4, and §8 below describe the eventual shape, not current code.
>
...
4. **Extensibility is a first-class primitive.** Hooks/actions/filters (WordPress) + events/listeners (Laravel) + a plugin & theme model (Loom). This is what makes Volo a *platform you build anything onto*, not just a framework. See §3.5.
5. **One operations layer, three equal surfaces: MCP · UI · CLI.** Every capability is an *operation* in a single core layer; CLI, Studio UI, and the **Volo MCP server** are thin front-ends. This is what makes "agent-first, CLI/UI optional" real.
6. **Zero-config local → scale substrate for prod.** `volo new` runs on nothing — embedded SQLite, DB-backed queue/cache, dev mail-catcher. Move to Postgres (+ optional edge drivers) for scale; the in-house APIs never change.
...
- **One `SqlDatabase` surface, four substrate drivers — the tier picks one, the app never changes.** Node: `openPostgres` over `pg` (`@volo/pg`) at scale, `openSqlite` in dev. Cloudflare Workers (no filesystem, no node sockets): `d1ToSqlDatabase` over D1 (the edge's SQLite) or `hyperdriveToSqlDatabase` over **Cloudflare Hyperdrive** (the edge's Postgres — pooled, connection-cached, fronting a real Postgres), both in `@volo/cloudflare`. Same async `SqlDatabase` seam, same `?`→`$n` dialect (shared with `@volo/pg`), so a DB-driven page runs the identical query path on every tier.
- **Managed "Volo Cloud" is a later commercial layer** that one-click-provisions Postgres + edges — the Vercel→Next / Forge→Laravel / .com→.org model.
...
> **Status:** not built in v1. The `@volo/hooks`/`@volo/config` prototypes were
...
Each capability = an in-house Volo API on the one DB; thin drivers only at the edges.

| Capability | Substrate (default) | Volo API | Status |
|---|---|---|---|
...
| **Deploys** | rolling / zero-downtime; graceful drain | `volo deploy` | ◻ build |
...
**Browser→server stitching is shipped, not aspirational.** The server stamps the in-flight request span's W3C `traceparent` into a `<meta name="volo-traceparent">` on every dynamically rendered page; `@volo/observability`'s browser RUM client (`startBrowserRum`, imported by the synthesized `@volo/assets` client entry from the node-free `@volo/observability/rum` subpath) reads it, **adopts the server trace id**, and emits browser spans for navigation timing, same-origin resource fetches, and web vitals (LCP, INP, and raw per-shift `layout-shift` scores — not a summed CLS) — PII-free (same-origin paths + timing numbers only) and bounded-sampled. Those spans POST to the built-in `POST /__volo/browser-spans` receiver (`@volo/web`), which routes them through `traces.seams.onBrowserSpan` to the **same OTLP exporter** the server spans use, so a page load's browser spans land **under the server `http.request` span, one traceId**. Outbound `@volo/client` data fetches carry a `traceparent` (`wrapFetch`) on same-origin requests, so the API handler joins the same trace — UI → API → DB, unbroken. Proven end-to-end in `packages/integration/test/rum.integration.test.ts` against a local OTLP collector.
...
8. **Docks content on Tracks + Studio as admin; OTel; Volo MCP server (the unifying operations surface).**

---

## 9. Open questions to resolve

- **Postgres vs SQLite default for `volo new`** — SQLite for true zero-config (Rails-8 stance), Postgres the moment you need pub/sub-realtime, pgvector, or scale. Likely: SQLite local, Postgres prod, one API over both.
...
- **Managed Volo Cloud** — scope/timing of the commercial layer.
