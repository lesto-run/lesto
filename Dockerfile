...
# ---- Production image for a Keel app ----------------------------------------
# A Keel app is a single `keel.app.ts` that default-exports its AppConfig; the
# `keel` CLI boots it over HTTP (`keel serve`) and drains its durable, SQL-backed
...
# app needs only @keel/* and the database driver.
FROM base AS deps
...
# The stateless web tier listens here. `keel serve` defaults to 3000; override
...
# Boot the HTTP server in front of the app. `keel serve` calls `createApp`,
...
CMD ["sh", "-c", "bunx keel serve --port ${PORT}"]
