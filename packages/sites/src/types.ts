...
/**
 * A response body the prerenderer can capture, mirroring `@lesto/web`'s `LestoBody`.
 *
 * We restate the union here rather than depend on `@lesto/web`: `RenderResponse`
 ...
 */
export type LestoResponseBody = string | Uint8Array | ReadableStream<Uint8Array> | undefined;

/**
 * The slice of a response the prerenderer needs.
 *
 * `@lesto/web`'s `LestoResponse` satisfies this structurally, so the app's own
...
export interface RenderResponse {
  ...
  /**
   * The response body, in any arm `@lesto/web` may produce. `prerenderSite`
   * drains it to a `string` before it becomes a {@link RenderedPage}'s `html`.
   */
  readonly body: LestoResponseBody;
}
