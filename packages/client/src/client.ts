/**
 * `createApi` — a typed, browser-safe fetch client for a Keel app's data routes.
 *
 *   // The contract: the wire types, declared ONCE and shared by import (no
 *   // codegen, no GraphQL). In a real app these reference the same `@keel/db`
 ...
 * required and typed via `@keel/router`'s `PathParams` — all by inference over
