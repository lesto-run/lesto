...
async function updatePackageJson(cwd: string, packageJson: PackageJson | undefined): Promise<void> {
  ...
  // Add @lesto/content-core to dependencies if not present
  pkg.dependencies = pkg.dependencies ?? {};
  if (!pkg.dependencies["@lesto/content-core"]) {
    pkg.dependencies["@lesto/content-core"] = "^0.0.1";
    console.log("  Added @lesto/content-core to dependencies");
  }
...
async function updateViteConfig(cwd: string, viteConfigFile: string): Promise<void> {
  ...
  if (content.includes("@lesto/content-vite-plugin") || content.includes("docks(")) {
  ...
  const updatedContent = content
    ...
    .replace(
      /(import .* from ['"]vite['"];?)/,
      '$1\nimport docks from "@lesto/content-vite-plugin";',
    )
  ...
  if (updatedContent !== content) {
    ...
    console.log('    import docks from "@lesto/content-vite-plugin";');
...
async function updateNextConfig(cwd: string, nextConfigFile: string): Promise<void> {
  ...
  if (content.includes("@lesto/content-next") || content.includes("withDocks")) {
  ...
  console.log('    import { withDocks } from "@lesto/content-next";');
...
export async function runInit(cwd: string, options: InitOptions = {}): Promise<void> {
  ...
  const configContent = `import { defineConfig, defineCollection } from "@lesto/content-core";
import { z } from "zod";
