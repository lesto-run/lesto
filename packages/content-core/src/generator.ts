 ...
 */
function preprocessForSerialization(
  obj: unknown,
...
function serializeEntries(entries: RuntimeEntry[], output?: OutputConfig): string {
  ...
  return `// Generated by @keel/content-core - DO NOT EDIT
...
function generateIndex(collections: string[]): string {
  const imports = collections.map((name) => `import ${name} from "./${name}.js";`).join("\n");
...
function generateDataTypes(collections: AnyCollection[]): string {
  const lines: string[] = [
    "// Generated by @keel/content-core - DO NOT EDIT",
    "",
    'import type { CollectionEntry, CollectionRegistry } from "@keel/content-core";',
    "",
  ];

  for (const col of collections) {
    lines.push(`export declare const ${col.name}: CollectionEntry<"${col.name}">[];`);
  }

  lines.push("");
  lines.push("export declare const collections: {");
  for (const col of collections) {
    lines.push(`  ${col.name}: CollectionEntry<"${col.name}">[];`);
  }
  lines.push("};");
  lines.push("");
...
export async function generate(options: GenerateOptions = {}): Promise<GenerateResult> {
  ...
  for (const col of engine.getCollections()) {
    ...
    const colConfig = collections.find((c) => c.name === col.name);
    const content = serializeEntries(col.entries, colConfig?.output);
    writePromises.push(writeFile(path.join(outDir, `${col.name}.js`), content, "utf-8"));
  ...
  const moduleTypesContent = generateTypes(collections, taxonomies, (relativeConfigPath === undefined ? {} : { configPath: relativeConfigPath }));
...
export function watch(options: WatchOptions = {}): WatchHandle {
  ...
  async function writeSharedFiles(): Promise<void> {
    ...
    const collectionNames = engine.getCollections().map((c) => c.name);
    const writePromises: Promise<void>[] = [];
