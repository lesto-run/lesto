...
const fillersWords = fillersList as string[],
  hedgesWords = hedgesList as string[],
  weaselsWords = weaselsList as string[];
const cussWords = cuss as Record<string, number>;
const escapeRegex = (s: string): string => s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
const wordListPattern = (words: string[]): RegExp =>
...
const P = {
  fillers: wordListPattern(fillersWords),
  weasels: wordListPattern(weaselsWords),
  hedges: wordListPattern(hedgesWords),
  condescending: wordListPattern(["simply", "obviously", "clearly", "easily", "of course"]),
  repeated: /\b(\w+)\s+\1\b/gi,
  simplify: wordListPattern(Object.keys(SIMPLIFY_MAP)),
  word: /\b\w+\b/gi,
  passive:
    /\b((?:am|are|were|being|is|been|was|be)\s+(?:\w+ed|built|made|done|given|taken|seen|known|found|told|shown|written|broken|chosen|spoken|stolen|driven|forgotten|hidden|begun|sung|run|gone|sent|spent|lent|lost|left|kept|felt|held|met|paid|said|sold|read|heard|thought|bought|brought|caught|taught|fought|sought|worn|torn))\b/gi,
  adverbs: /\b(\w+ly)\b/gi,
  spelling: /\b([a-zA-Z]+(?:'[a-zA-Z]+)?)\b/g,
} as const;
...
class RuleContext {
  ...
  diag(
    ...
    message: string,
    matchText: string,
    extra?: Partial<Pick<Diagnostic, "length" | "fix" | "help" | "suggestion" | "prompt">>,
  ): Diagnostic {
    const { line, column } = offsetToPositionFast(this.lineIndex, offset);
    const id = `${rule}-${offset}-${matchText
      .slice(0, 20)
      .toLowerCase()
      .replace(/\s+/g, "_")
      .replace(/[^a-z0-9_]/g, "")}`;
...
const getRemovalRange = (spanText: string, i: number, len: number, off: number) => {
  const e = i + len;
  const [s, end] =
    spanText[e] === " " ? [i, e + 1] : i > 0 && spanText[i - 1] === " " ? [i - 1, e] : [i, e];
...
const patternRule =
  ...
  (spans, file, lineIndex) => {
    ...
    return spans.flatMap((span) =>
      [...ctx.scan(span.text, pat, span.offset)].map((m) => {
        const fix = remove
          ? { ...getRemovalRange(span.text, m.index, m.text.length, span.offset), text: "" }
          : undefined;
        return ctx.diag(
          name,
          m.offset,
          msg(m.text),
          m.text,
          ctx.withHelp(name, m.text, fix === undefined ? {} : { fix }),
        );
      }),
    );
  };
...
export const fillers: Rule = patternRule(
  "fillers",
  P.fillers,
  (w) => `"${w}" is a filler word`,
  "warning",
  true,
);
...
export const condescending: Rule = patternRule(
  "condescending",
  P.condescending,
  (w) => `"${w}" can be condescending`,
  "warning",
  true,
);
export const passive: Rule = patternRule(
  "passive",
  P.passive,
  (w) => `"${w}" may be passive voice`,
);
...
export const repeated: Rule = (spans, file, lineIndex) => {
  ...
  return spans.flatMap((span) =>
    [...ctx.scan(span.text, P.repeated, span.offset)].flatMap((m) => {
...
export const simplify: Rule = (spans, file, lineIndex) => {
  ...
  return spans.flatMap((span) =>
    [...ctx.scan(span.text, P.simplify, span.offset)].flatMap((m) => {
      ...
      return [
        ctx.diag(
          "simplify",
          m.offset,
          `"${m.text}" can be simplified to "${r}"`,
          m.text,
          ctx.withHelp("simplify", m.text, {
...
export const profanity: Rule = (spans, file, lineIndex) => {
  const ctx = new RuleContext(file, lineIndex, "error");
  return spans.flatMap((span) =>
    [...ctx.scan(span.text, P.word, span.offset)]
      ...
      .map((m) => {
        ...
        return ctx.diag(
          "profanity",
          m.offset,
          `"${m.text}" is profane${s === 2 ? " (severe)" : ""}`,
          m.text,
          ctx.withHelp("profanity", m.text),
        );
      }),
  );
};
...
export const adverbs: Rule = (spans, file, lineIndex) => {
  const ctx = new RuleContext(file, lineIndex, "warning");
  return spans.flatMap((span) =>
    [...ctx.scan(span.text, P.adverbs, span.offset)]
...
export const cliches: Rule = (spans, file, lineIndex) => {
  const ctx = new RuleContext(file, lineIndex, "warning");
  return spans.flatMap((span) =>
    (noCliches(span.text) as Array<{ index: number; offset: number }>).map((m) => {
      const txt = span.text.slice(m.index, m.index + m.offset),
        off = span.offset + m.index;
...
export const readability: Rule = (spans, file, lineIndex) => {
  const ctx = new RuleContext(file, lineIndex, "warning");
  return spans.flatMap((span) =>
    tokenizeSentences(span.text, span.offset)
      .filter((s) => s.wordCount > READABILITY_MIN_WORDS)
      .flatMap((s) => {
        ...
        return [
          ctx.diag(
            ...
            msg,
            s.text,
            ctx.withHelp("readability", s.text, { length: s.text.length }),
          ),
        ];
      }),
  );
...
export const ruleRegistry: Record<RuleName, Rule> = {
  fillers,
  weasel,
  hedge,
  condescending,
