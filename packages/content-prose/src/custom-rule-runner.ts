...
import type {
  LimitRule, ConsistentRule, FirstUseRule, CasingRule,
  BanRule, ReplaceRule, MatchRule, CustomFunctionRule,
...
class RuleContext {
  ...
  diag(
    ...
    const diagnostic: Diagnostic = { id, rule, message, file: this.file, offset, line, column, severity: this.severity };
  ...
  *scan(text: string, pattern: RegExp, baseOffset = 0) {
  ...
  static getUnits(spans: TextSpan[], scope: 'document' | 'paragraph' | 'sentence' = 'document') {
    ...
    return spans.flatMap(s => scope === 'paragraph'
  ...
  private static splitParagraphs(text: string, base: number) {
    ...
    for (const m of text.matchAll(/\n\s*\n/g)) {
      ...
      if (para) result.push({ text: para, offset: base + lastEnd + (raw.length - raw.trimStart().length) });
      lastEnd = m.index! + m[0].length;
    }
    const lastRaw = text.slice(lastEnd);
    const last = lastRaw.trim();
    if (last) result.push({ text: last, offset: base + lastEnd + (lastRaw.length - lastRaw.trimStart().length) });
    return result;
  }
}
...
function calcFix(fix: FixDef | undefined, spanText: string, matchIdx: number, matchText: string, spanOffset: number): Diagnostic['fix'] {
  ...
  switch (fix.type) {
    ...
    case 'transform': {
      ...
      const text = t === 'lowercase' ? matchText.toLowerCase()
        : t === 'uppercase' ? matchText.toUpperCase()
...
function preserveCase(orig: string, repl: string): string {
  const origFirst = orig[0], replFirst = repl[0];
...
function buildPattern(pattern: string, isRegex: boolean, ignoreCase = true, wordBound = true): RegExp {
...
export function runLimitRule(rule: LimitRule, spans: TextSpan[], file: string, lineIndex: LineIndex): Diagnostic[] {
  ...
  if (scope !== 'document') {
    return RuleContext.getUnits(spans, scope).flatMap(unit => {
      ...
      if (max !== undefined && count > max && matches[max]) {
        const m = matches[max];
        const msg = rule.message.replace('$count', String(count)).replace('$max', String(max)).replace('$match', m.text);
        results.push(ctx.diag(rule.name, m.offset, msg, m.text, { help, suggestion }));
      }

      if (count < min) {
        const msg = rule.message.replace('$count', String(count)).replace('$min', String(min)).replace('$match', pattern);
...
export function runConsistentRule(rule: ConsistentRule, spans: TextSpan[], file: string, lineIndex: LineIndex): Diagnostic[] {
  const ctx = new RuleContext(file, lineIndex, rule.severity ?? 'warning');
...
export function runFirstUseRule(rule: FirstUseRule, spans: TextSpan[], file: string, lineIndex: LineIndex): Diagnostic[] {
  const ctx = new RuleContext(file, lineIndex, rule.severity ?? 'warning');
  ...
  return spans.flatMap(span => {
    const hasExpansion = (idx: number, len: number) =>
      /^\s*\([^)]+\)/.test(span.text.slice(idx + len)) || /\([^)]+\)\s*$/.test(span.text.slice(0, idx));
    ...
    return [...ctx.scan(span.text, regex, span.offset)].flatMap(m => {
      ...
      return [ctx.diag(rule.name, m.offset, rule.message.replace('$match', m.text), m.text, {
...
const SMALL_WORDS = new Set(['a', 'an', 'the', 'and', 'but', 'or', 'for', 'nor', 'on', 'at', 'to', 'by', 'of', 'in']);
...
const CASE_FIXERS: Record<CaseType, (w: string, i: number) => string> = {
  ...
  title: (w, i) => i > 0 && SMALL_WORDS.has(w.toLowerCase())
    ? w.toLowerCase()
...
export function runCasingRule(rule: CasingRule, spans: TextSpan[], file: string, lineIndex: LineIndex): Diagnostic[] {
  const ctx = new RuleContext(file, lineIndex, rule.severity ?? 'warning');
  ...
  return spans.flatMap(span => {
    ...
    return [ctx.diag(rule.name, span.offset, rule.message.replace('$match', span.text), span.text, {
...
export function runBanRule(rule: BanRule, spans: TextSpan[], file: string, lineIndex: LineIndex): Diagnostic[] {
  const ctx = new RuleContext(file, lineIndex, rule.severity ?? 'warning');
...
export function runReplaceRule(rule: ReplaceRule, spans: TextSpan[], file: string, lineIndex: LineIndex): Diagnostic[] {
  const ctx = new RuleContext(file, lineIndex, rule.severity ?? 'warning');
...
export function runMatchRule(rule: MatchRule, spans: TextSpan[], file: string, lineIndex: LineIndex): Diagnostic[] {
  const ctx = new RuleContext(file, lineIndex, rule.severity ?? 'warning');
  ...
  if (rule.negate) {
    ...
    return [ctx.diag(rule.name, offset, rule.message.replace('$match', rule.pattern), rule.pattern, {
      help: rule.help,
      suggestion: rule.suggestion,
    })];
  }
...
async function loadCustomFunction(functionPath: string, basePath?: string): Promise<Rule> {
  ...
  try {
    ...
    throw new Error(`Failed to load custom rule from '${functionPath}': ${e instanceof Error ? e.message : e}`, { cause: e });
  }

  const fn = typeof mod.default === 'function' ? mod.default
    : typeof mod.rule === 'function' ? mod.rule
    : null;
  if (!fn) throw new Error(`Custom rule module '${functionPath}' must export a default function or a function named 'rule'`);
...
export async function runCustomFunctionRule(
  ...
  try {
    ...
    throw new Error(`Custom rule '${rule.name}' failed: ${e instanceof Error ? e.message : e}`, { cause: e });
  }

  if (!Array.isArray(diagnostics)) {
    throw new Error(`Custom rule '${rule.name}' must return an array of diagnostics, got ${typeof diagnostics}`);
  }
