...
function formatHelpSection(diagnostic: Diagnostic): string | null {
  ...
  if (diagnostic.help) {
    const helpText =
      diagnostic.help.length > 100 ? diagnostic.help.slice(0, 97) + "..." : diagnostic.help;
 ...
 */
function formatRichDiagnostic(diagnostic: Diagnostic, sources: Map<string, string>): string {
  ...
  const underlineLength = inferUnderlineLength(
    diagnostic,
    snippetLines[primaryLineIndex]?.content ?? "",
  );
  const { processedLines, displayColumn } = processSnippetLines(
    snippetLines,
    diagnostic,
    underlineLength,
  );
  ...
  output.push(
    `  ${pc.dim(pc.cyan(BOX.TOP_LEFT + BOX.HORIZONTAL))}${pc.dim("[")}${pc.underline(fileLocation)}${pc.dim("]")}`,
  );
...
function formatRichIssueCount(errorCount: number, warningCount: number): string {
  ...
  const parts: string[] = [];
  if (warningCount > 0)
    parts.push(pc.yellow(`${warningCount} ${pluralize(warningCount, "warning")}`));
