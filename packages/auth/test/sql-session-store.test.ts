 ...
 * sweeps). The real-engine, two-driver proof is `@keel/integration` (item 7).
 */
...
function makeFakeDb(options: { expiresAsString?: boolean } = {}): {
  ...
  const prepare = (sql: string): SqlStatement => {
    if (sql.includes("INSERT INTO keel_sessions")) {
      return {
        run: async (params = []) => {
...
describe("installSessionSchema", () => {
  it("issues the table + two indexes, all IF NOT EXISTS (idempotent)", async () => {
    ...
    expect(create).toContain("CREATE TABLE IF NOT EXISTS keel_sessions");
    // BIGINT, not INTEGER — epoch-ms overflows PG int4.
    expect(create).toContain("expires_at BIGINT NOT NULL");
    expect(userIndex).toContain("CREATE INDEX IF NOT EXISTS keel_sessions_user_id");
    expect(expiresIndex).toContain("CREATE INDEX IF NOT EXISTS keel_sessions_expires_at");
  });
});
