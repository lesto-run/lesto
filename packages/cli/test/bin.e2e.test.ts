...
/**
 * The `keel` bin, end to end as a REAL spawned process (operability-dx #6).
 *
 ...
 * project's `keel.app.ts` / `keel.sites.ts`, the SIGTERM/SIGINT shutdown handlers,
 ...
 *
 *   - `routes`            — loads `keel.app.ts`, prints routes, exits 0
 ...
 * completion. The fixture root carries a `keel.app.ts` (+ a `keel.sites.ts`),
...
beforeAll(async () => {
  ...
  workspace = await mkdtemp(join(tmpdir(), "keel-cli-e2e-"));
});
...
describe("bin e2e", () => {
  it("routes: loads the project's keel.app.ts and prints its routes, exiting 0", async () => {
