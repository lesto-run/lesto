...
//
// The overlay is built with NO `dispatchDevTool` — the live read-tool dispatch (ADR 0033 Inc 6b,
// L-e7ea34e3) is set IN PLACE by `startDevMcp` once the loopback dev MCP server is up (and cleared
// on teardown). While it is absent (server not yet bound, torn down, or a non-dev command), the
// bridge's OWN missing-seam arm (`dispatchAiTurn`) fails closed → the overlay's tested inspect-only
// "not available" reply. So the overlay lights up exactly when the governed dev MCP plane is
// running, the production fail-closed path IS the tested bridge branch (not a bin duplicate), and
// it never mutates (the positive read-only allowlist runs before the seam — `ai-bridge.ts`).
...
const startDevMcp: CliDeps["startDevMcp"] =
  command === "dev"
    ? async ({ app, routes, devState: ring }) => {
        ...
        // Wire the in-preview AI overlay's LIVE dispatch (ADR 0033 Inc 6b) IN PLACE on the overlay
        // seam: the same governed, audited tool set the loopback transport serves, run IN-PROCESS
        // over the SAME live context (so `describe_app` reflects a hot-reloaded route). The turn's
        // tool is already allowlist-checked by the bridge before this runs, and `dispatch` audits
        // every call — so the overlay is exactly an in-process read-only client of the governed dev
        // MCP plane. Setting it here (rather than a wrapping closure) keeps the bridge's OWN
        // missing-seam arm as the tested production fail-closed path. `{}` args on purpose: the
        // Phase-1 `DEV_INSPECT_TOOL` (`describe_app`) is input-less; the turn's redacted page
        // context is carried for future ACTING tools, not read-tool arguments.
        const tools = buildTools(context);
        if (aiOverlay !== undefined) {
        ...
        return {
          close: async () => {
            // Clear the overlay's dispatch first, so a turn racing teardown fails closed (the
            // bridge's missing-seam arm) rather than dispatching against a torn-down context.
            if (aiOverlay !== undefined) aiOverlay.dispatchDevTool = undefined;
