 ...
 * touching a disk.
 *
 * The app they emit is small but REAL — it boots on `@volo/kernel`'s
 * `createApp` with a `posts` table (defined as a `@volo/db` value), a
 * migration that runs `createTableSql(posts)`, and a code-first `volo()`
 * app whose handlers query via `@volo/db`. Mirrors the shape of the
 * canonical blog example (`examples/blog`) and the rest of the
 * post-ADR-0004 codebase.
 */

/**
 * The `@volo/*` packages a scaffolded app depends on, by directory name under
 * `packages/`.
 *
 * `@volo/cli` is the `volo` binary `volo dev`/`build` runs (its absence was a
 * silent break — blocker #9); `@volo/assets` is the client-bundle pipeline the
 * island build needs in the app's graph; `@volo/cloudflare` is the edge adapter
 * the deploy template's `worker.ts` fronts the app with (`volo deploy
 * --cloudflare`). The rest are the app's own runtime (db/kernel/migrate/runtime,
 * the web router, the UI engine).
 */
export const VOLO_PACKAGES = [
  "@volo/cli",
  "@volo/assets",
  "@volo/cloudflare",
  "@volo/db",
...
/**
 * Resolve a `@volo/*` package name to the dependency specifier the scaffold pins
 * it at.
 *
 ...
 * `create-volo --local` for in-monorepo dev before anything is published). The
 * resolver is injected so a test can pin to a fake specifier and the publish
 * decision lives in exactly one place.
 */
export type VoloDepResolver = (pkg: (typeof VOLO_PACKAGES)[number]) => string;

/**
 * `package.json` for the scaffolded app: starter deps + a `volo dev` script.
 *
 * The `@volo/*` specifiers come from the injected `voloDep` resolver (a `file:`
 ...
 * see `volo.app.ts`'s `ui: { dialect: "preact" }`).
 */
export function packageJson(name: string, voloDep: VoloDepResolver): string {
  const voloDeps = Object.fromEntries(VOLO_PACKAGES.map((pkg) => [pkg, voloDep(pkg)]));
  ...
  const manifest = {
    ...
    type: "module",
    private: true,
    // A scaffolded app is the user's own private project, not a Volo package — so
    // it declares `UNLICENSED` rather than inheriting Volo's MIT. The author
...
/**
 * `volo.app.ts` — the project root's entrypoint.
 *
 * It default-exports a `VoloAppConfig` (the `{ db, app, migrations }` object
 * `createApp` accepts), which `@volo/cli`'s `volo dev` loads and boots.
 */
export function voloApp(): string {
  return `/**
 * The app: a \`posts\` table, its migration, and a code-first \`volo()\` app,
 * wired into the VoloAppConfig that \`volo dev\` boots. This is the whole
 * application — grow it by adding tables, migrations, routes, and pages.
 *
 * Routes read top-to-bottom like Hono/Express: one \`volo()\` surface for both
...
import { createDb, createTableSql, defineTable, dropTableSql, integer, text } from "@volo/db";
...
// types. One \`volo()\` surface for the whole app; grow it by chaining more
...
function buildApp(db: Db) {
  return (
    volo()
      ...
      // The hydration runtime: \`volo build\`/\`dev\` bundle \`app/islands/\` into
      ...
      .page("/", {
        component: () =>
          createElement(
            "main",
            null,
            createElement("h1", null, "Welcome to Volo"),
...
// The driver seam: \`@volo/runtime\`'s \`openSqlite\` boots better-sqlite3 under
...
// runs migrations) and the typed \`@volo/db\` the handlers query through.
const { db: handle } = await openSqlite("volo.db");
...
const config: VoloAppConfig = {
  ...
  // \`react\`→\`preact/compat\` alias (read by \`volo dev\`/\`build\`) and — for a
 ...
 *
 * One `defineIsland` default-export per file is the convention `volo build`/`dev`
...
/**
 * `volo.sites.ts` — the project's declared sites, mirroring `volo.app.ts`.
 *
 * `volo build`/`dev` read its default export. A single dynamic site at `/` runs
 * the whole app live; grow it (static zones, multiple origins) as the app does.
 * Its presence is also what keeps `volo build` from crashing — a missing
 * `volo.sites.ts` is tolerated (app-only dispatch), but shipping one is the
 * starting point every real app grows from.
 */
export function voloSites(): string {
  return `import type { Site } from "@volo/sites";
...
export function tsconfig(): string {
  const config = {
    ...
    include: ["volo.app.ts", "volo.sites.ts", "worker.ts", "app"],
  };
 ...
 *
 * This builds its OWN minimal `volo()` app rather than importing `volo.app.ts` —
 * the same edge-twin split `examples/estate` makes — because `volo.app.ts` opens a
 ...
 * `buildApp(db)` over `@volo/db`'s D1 adapter (see `examples/estate`).
 *
 * The deferred island still hydrates: `volo build` shipped the Preact `/client.js`
 ...
 *
 *   volo deploy --cloudflare        # builds out/, then `wrangler deploy`
 */
...
// The edge twin of \`volo.app.ts\`'s home page — the same client module tag and
...
/**
 * `wrangler.jsonc` — the Cloudflare deploy config (`volo deploy --cloudflare`).
 *
 * The minimal wiring: `nodejs_compat` (the app's runtime leans on `node:*`), the
 * `worker.ts` entry, and the Static Assets binding rooted at `out/` (what `volo
...
export function wranglerConfig(name: string): string {
  return `{
	// Generated by \`create-volo\`. The minimal Cloudflare deploy config; see the
	// README's "Deploy to Cloudflare" section. Run: volo deploy --cloudflare
	...
	// The prerendered static site (\`volo build\` → out/): served first at the PoP,
...
\`volo dev\` loads \`volo.app.ts\` (which default-exports the app config) and
boots it: the kernel runs migrations and stands up request dispatch over
the typed \`@volo/db\` handle the route handlers query through.

## Deploy to Cloudflare

\`\`\`sh
volo deploy --cloudflare
\`\`\`
...
\`@volo/cloudflare\`'s \`toFetchHandler\` + \`withAssets\`, serving the prerendered
files first and the live app second.

A Worker has no filesystem SQLite, so \`worker.ts\` builds a minimal edge twin
of the home page (the hydrating island) rather than importing \`volo.app.ts\`
...
the \`examples/estate\` reference in the Volo source) and build the full app over
\`@volo/db\`'s D1 adapter. Edit \`wrangler.jsonc\`'s \`name\` to pick your
...
- \`volo.app.ts\` — the whole app: a \`posts\` table (defined via
  \`@volo/db\`'s \`defineTable\`), its migration, and a code-first \`volo()\`
...
- The \`POST /posts\` handler validates the request body at the boundary with
  ...
  See \`docs/adr/0005-validation-at-the-boundary.md\` in the Volo source.
