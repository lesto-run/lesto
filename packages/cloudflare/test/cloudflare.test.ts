 ...
 * in for `@lesto/observability`'s request tracer. Each minted span carries `data`
...
/**
 * A parser standing in for `@lesto/observability`'s `parseTraceparent`: it returns
...
describe("toFetchHandler", () => {
  ...
  it("passes response headers through — a Set-Cookie survives to the browser", async () => {
    const { dispatch } = recordingDispatch({
      status: 302,
      body: "",
      headers: { "set-cookie": "__Host-lesto_session=abc; Secure", location: "/mls" },
    });
    ...
    expect(response.headers.get("set-cookie")).toBe("__Host-lesto_session=abc; Secure");
  ...
  it("publishes the request span on the context, so a seam parents on it", async () => {
    ...
    // how a @lesto/db onQuery seam would find the request span to parent its child.
...
describe("toFetchHandler — hardening (edge parity)", () => {
  ...
  it("maps a coded transport error to its status, without logging a client error", async () => {
    ...
    const response = await toFetchHandler(
      throwingDispatch(new LestoError("RUNTIME_BODY_TOO_LARGE", "too big")),
      { logError: (_message, error) => logged.push(error) },
...
describe("toFetchHandler — timeoutMs", () => {
  ...
  it("propagates a dispatch rejection that loses the race to the error boundary", async () => {
    ...
    const response = await toFetchHandler(
      rejectingDispatch(new LestoError("RUNTIME_BODY_TOO_LARGE", "too big")),
      {
        timeoutMs: 1000,
        logRequest: () => undefined,
        logError: () => undefined,
...
describe("wranglerConfig", () => {
  ...
  it("emits alias, d1_databases, vars, and placement when supplied", () => {
    ...
    expect(config.vars).toEqual({ LESTO_DEMO: "1" });
...
describe("serializeWranglerConfig", () => {
  ...
  it("regenerates the committed examples/estate/wrangler.jsonc byte-identically", () => {
    ...
    const text = serializeWranglerConfig(config, {
      fields: {
        name: [
          "Generated by `wranglerConfig` (@lesto/cloudflare) from the deploy plan, then",
        ...
        assets: [
          "The prerendered static marketing site (`lesto build` → out/marketing).",
        ...
        alias: [
          ...
          "every React specifier to Preact's compat layer, and `worker.ts` renders",
          "through `preactServerRenderer`, so the SSR'd markup and the shipped client",
          "speak one dialect. This set is a SUPERSET of `build-client.ts --preact`'s",
          "client alias — it adds the `react-dom` and `react-dom/server` shims, because",
          "the worker imports the `@lesto/ui/server` surface (for `preactServerRenderer`)",
          "whose barrel drags `resources.ts` (bare-`react-dom` resource hints) and the",
          "page renderers (`react-dom/server`) into the graph. The client bundle never",
          "imports that server surface, so it needs neither shim. The worker resolves",
          "both but invokes neither on this Preact path — inert no-ops.",
        ],
      },
    });
