...
import { CreateLestoError, fileColonPin, scaffold } from "./index";

// `--local` pins `@lesto/*` deps at in-repo `file:` paths (in-monorepo dev / e2e)
// instead of the default published `^0.x` ranges. The project name is the first
// non-flag argument.
const argv = process.argv.slice(2);
const local = argv.includes("--local");
const name = argv.find((arg) => !arg.startsWith("-")) ?? "lesto-app";
...
try {
  const files = await scaffold(
    local ? { name, targetDir, lestoDep: fileColonPin } : { name, targetDir },
    io,
  );
  console.log(
