...
/** Pull the `data-lesto-island` ids out of rendered HTML, in order. */
function islandIdsIn(html: string): string[] {
  return [...html.matchAll(/data-lesto-island="([^"]*)"/g)].map((m) => m[1] as string);
}
...
// ---------------------------------------------------------------------------
// assertClientDef: the shared union rules both declaration paths enforce.
// ---------------------------------------------------------------------------

describe("assertClientDef", () => {
  it("accepts a well-formed eager def", () => {
    expect(() => assertClientDef(Account)).not.toThrow();
  });
...
describe("Registry client components", () => {
  ...
  it("uses console.warn by default for a cross-namespace shadow", () => {
    ...
    expect(spy.mock.calls[0]?.[0]).toContain("[lesto/ui]");
    ...
    expect(page.islands).toEqual([
      {
        ...
        props: {},
        ssr: false,
        bind: { session: { source: "session", href: "/__lesto/data/session" } },
      },
    ]);
  });
...
describe("renderPage", () => {
  it("renders an island wrapper with its fallback and records the manifest", () => {
    ...
    expect(html).toBe(
      '<div class="box">hi <div data-lesto-island="$.children[1]">' +
  ...
  it("renders an island with no props and no fallback as an empty shell", () => {
    ...
    expect(renderToStaticMarkup(page.element)).toBe('<div data-lesto-island="$"></div>');
  ...
  it("renders an ssr island's REAL component server-side and flags ssr in the manifest", () => {
    ...
    expect(html).toBe(
      '<div class="box"><div data-lesto-island="$.children[0]"><strong>live</strong></div></div>',
    );
