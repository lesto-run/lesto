...
function call(handler: ReturnType<typeof browserSpansHandler>, body: unknown): AnyKeelResponse {
...
describe("browserSpansHandler", () => {
  ...
  it("refuses an oversized payload with a coded 413, never reaching the sink", () => {
    ...
    expect(response.headers["x-keel-error"]).toBe("WEB_BROWSER_SPANS_BODY_TOO_LARGE");
    expect(seen).toEqual([]);
  });
