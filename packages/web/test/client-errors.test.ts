 ...
 * {@link AnyLestoResponse}; we narrow the wide `Handler` return for the assertions.
 */
function call(handler: ReturnType<typeof clientErrorsHandler>, body: unknown): AnyLestoResponse {
...
describe("clientErrorsHandler", () => {
  ...
  it("refuses an oversized beacon with a coded 413, never reaching the sink", () => {
    ...
    expect(response.headers["x-lesto-error"]).toBe("WEB_CLIENT_ERROR_BODY_TOO_LARGE");
    expect(seen).toEqual([]);
  });
