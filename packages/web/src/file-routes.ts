...
export function generateRouteManifest(
  ...
  // emitting a manifest that bundles cleanly but throws when it is applied.
  compileFileRoutes(files);
  ...
  const sorted = [...files].toSorted((a, b) => {
    const ka = routeKey(a.kind, a.segments);
    const kb = routeKey(b.kind, b.segments);
    return ka < kb ? -1 : 1;
  });
