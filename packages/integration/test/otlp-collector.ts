 ...
 * exactly the slice of the OTLP/HTTP JSON protocol Lesto's `OtlpHttpExporter`
 ...
 * Mirrors `@lesto/observability`'s `SpanData` plus the resource's `service.name`,
...
export interface OtlpCollector {
  /** The endpoint to set as `LESTO_OTLP_URL`, e.g. `http://127.0.0.1:54321/v1/traces`. */
  readonly url: string;
 ...
 * `LESTO_OTLP_URL`. Every `POST /v1/traces` is decoded and appended to `spans`;
