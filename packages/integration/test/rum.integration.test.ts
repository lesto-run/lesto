 ...
 * from the SSR-injected `keel-traceparent` meta — must land in the SAME local
 * collector as the server `http.request` span, parented on it, under ONE traceId.
 *
 * It boots `@keel/runtime`'s `serve` with the env-driven tracer (exactly as
 * `keel serve` / estate construct it), renders a real page (so the server stamps
 ...
 * built-in `/__keel/browser-spans` receiver — wired to `traces.seams.onBrowserSpan`,
...
import { BROWSER_SPANS_ROUTE, currentRequestSpan, keel, runWithContext } from "@keel/web";
import { formatTraceparent, parseTraceparent, tracesFromEnv } from "@keel/observability";
import type { BrowserSpan, CurrentSpan, Traces } from "@keel/observability";
...
beforeAll(async () => {
  ...
  const built = tracesFromEnv(
    { KEEL_OTLP_URL: collector.url, KEEL_OTLP_SERVICE: "integration-rum" },
...
/** Read the `keel-traceparent` meta content out of an HTML document, as a browser would. */
function readTraceparentFromHtml(html: string): string | undefined {
  const match = html.match(/<meta name="keel-traceparent" content="([^"]+)"\s*\/?>/);
...
describe("browser spans stitch to the server trace (ARCHITECTURE.md §7)", () => {
  ...
  it("adopts the SSR traceparent meta the page emitted (the browser→server seam)", async () => {
    ...
    const app = keel().page("/p", { component: () => createElement("main", null, "x") });
