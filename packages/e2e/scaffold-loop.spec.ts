 ...
 *
 * It exercises the first five minutes of every Volo user:
 *
 *   create-volo <app>  →  volo build  →  volo dev  →  GET /  →  the island hydrates
 *
 * 1. `create-volo`'s own bin scaffolds a fresh app into a temp dir (the `file:`
 ...
 * 2. `volo build` produces the Preact island client (`out/client.js`) — a build
 *    that used to crash on a missing `out/` dir, and on the absent `volo.sites.ts`
 *    the scaffold now ships.
 * 3. `volo dev` boots and serves the page; we assert on the raw server HTML that
 ...
 * workspace `node_modules` (where every `@volo/*` package + `react`/`preact` is
 * present) — then runs the in-repo `volo` bin. That makes the app's own
 * `import "@volo/db"` resolve exactly as a published install would. At the `0.x`
...
const CREATE_VOLO_BIN = join(REPO_ROOT, "packages", "create-volo", "src", "bin.ts");
const VOLO_BIN = join(REPO_ROOT, "packages", "cli", "src", "bin.ts");

const PORT = 4188;
const BASE_URL = `http://127.0.0.1:${PORT}`;
const APP_NAME = "scaffold-loop-app";

// Serial + one worker: the suite scaffolds once and shares a single `volo dev`
...
test.beforeAll(async () => {
  workspace = await mkdtemp(join(tmpdir(), "volo-scaffold-loop-"));
  appDir = join(workspace, APP_NAME);

  // 1. Scaffold a fresh app via create-volo's own bin. `--local` pins the @volo/*
  ...
  await run("bun", [CREATE_VOLO_BIN, APP_NAME, "--local"], workspace);
  ...
  await run("bun", [VOLO_BIN, "build"], appDir);

  // 4. Boot `volo dev` against the scaffolded app on a private port.
  dev = spawn("bun", [VOLO_BIN, "dev", "--port", String(PORT)], { cwd: appDir, stdio: "pipe" });
