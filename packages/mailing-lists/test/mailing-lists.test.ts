...
// Test rig
//
// One in-memory SQLite per test, adapted to @lesto/db's `SqlDatabase` shape.
...
//
// Both @lesto/db and @lesto/queue now speak the same positional, async seam, so a
...
beforeEach(async () => {
  ...
  // The queue rides the SAME async adapter as @lesto/db — one connection, one
  // seam, and now one TYPE: `@lesto/queue` re-exports `@lesto/db`'s `SqlDatabase`,
