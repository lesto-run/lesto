...
describe("Engine", () => {
  ...
  it("persists a void step so it is not re-executed on resume", async () => {
    ...
    const row = database
      .prepare("SELECT result FROM lesto_workflow_steps WHERE run_id = ? AND step_key = ?")
  ...
  it("converges on the winner's value when a concurrent pass wins the step-journal race", async () => {
    ...
    const racing: SqlDatabase = {
      ...
      prepare: (sql) => {
        ...
        if (sql.includes("INSERT INTO lesto_workflow_steps") && !raced) {
          return {
            ...
            run: async (params) => {
              ...
              await db
                .prepare(
                  "INSERT INTO lesto_workflow_steps (run_id, step_key, result) VALUES (?, ?, ?)",
                )
...
describe("the async seam", () => {
  it("commits work done inside transaction(fn) and exposes it via the async get terminal", async () => {
    await db.transaction(async (tx) => {
      await tx
        .prepare("INSERT INTO lesto_workflow_steps (run_id, step_key, result) VALUES (?, ?, ?)")
