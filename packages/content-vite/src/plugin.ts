...
const DEFAULT_BUNDLE_LIMITS: Required<Omit<BundleSizeLimit, "failOnExceed">> = {
  clientMain: 400,
  clientTotal: 500,
  bannedPackages: ["@keel/content-core"],
};
...
export function docks(options: DocksPluginOptions = {}): Plugin {
  ...
  return {
    ...
    config(config: UserConfig) {
      // Configure @keel/content-content alias to point to generated directory
      ...
      const configPatch: Partial<UserConfig> = {
        resolve: {
          alias: {
            "@keel/content-content": resolvedOutDir,
          },
        },
        // Exclude @keel/content-content from Vite's dependency optimization
        // to prevent stale data after content regeneration
        optimizeDeps: {
          exclude: ["@keel/content-content"],
        },
      };
        ...
        server.middlewares.use((req, res, next) => {
          const handle = async (): Promise<void> => {
            ...
            try {
              ...
              const mod = (await server.ssrLoadModule("@keel/content-content")) as {
