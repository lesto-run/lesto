...
describe("errorClass", () => {
  it("prefers a KeelError's stable code (the deploy-skew signal)", () => {
    const error = new KeelError("UI_ISLAND_UNKNOWN_COMPONENT", "Account is gone");
...
describe("defaultOverlay", () => {
  it("appends a dismissible banner naming each event, with no PII", () => {
    ...
    const box = document.querySelector("[data-keel-error-overlay]");
    expect(box).not.toBeNull();

    const text = box?.textContent ?? "";
    expect(text).toContain("[keel] mount-error Account: TypeError");
    expect(text).toContain("[keel] hydrate (failed 1, missing 2)");
    expect(text).toContain("[keel] recoverable-error: MISMATCH");
  });
  ...
  it("renders an event missing its optional fields without leaking 'undefined'", () => {
    ...
    const text = document.querySelector("[data-keel-error-overlay]")?.textContent ?? "";
    expect(text).toBe("[keel] hydrate (failed 0, missing 0)");
  ...
  it("dismisses itself on click", () => {
    ...
    const box = document.querySelector<HTMLElement>("[data-keel-error-overlay]");
    box?.click();

    expect(document.querySelector("[data-keel-error-overlay]")).toBeNull();
  });
});
...
describe("reportClientErrors", () => {
  ...
  it("POSTs a recoverable-error event carrying only the error class", () => {
    ...
    reporter.onRecoverableError(new KeelError("UI_HYDRATION_MISMATCH", "secret"));
