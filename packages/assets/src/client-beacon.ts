 ...
 *     CLASS or coded `LestoError.code`, and COUNTS. Never `error.message` (it can
 ...
 * carries no `@lesto/assets` import, yet the code that ships is the exact code
 * this file's tests exercise (no string-vs-source drift).
 */

/** Where the beacon POSTs — the receiving route lives in `@lesto/web` (core-runtime). */
export const BEACON_PATH = "/__lesto/client-errors";
...
export interface BeaconEvent {
  ...
  /** The error's coded `LestoError.code` or its constructor name — never its message. */
...
/** The `HydrationResult` shape the beacon reads (a structural subset of `@lesto/ui`'s). */
...
export interface BeaconOptions {
  ...
  /** `true` in `lesto dev`: paint the overlay, POST nothing. Defaults to `false`. */
 ...
 *
 * A `LestoError` carries a stable `code` we prefer (`UI_ISLAND_UNKNOWN_COMPONENT`
...
export function errorClass(error: unknown): string {
  if (error !== null && typeof error === "object") {
    ...
    // A `LestoError`-style code is a SCREAMING_SNAKE identifier — never user data.
...
export function defaultOverlay(payload: BeaconPayload): void {
  const lines = payload.events.map((event) => {
    ...
    return `[lesto] ${event.kind}${where}${why}${counts}`;
  });

  const box = document.createElement("div");
  box.setAttribute("data-lesto-error-overlay", "");
  box.style.cssText =
