...
That mechanism is no longer available — `@keel/db`'s rows are plain
...
### Why Zod (and not "@keel/validate" or another library)
...
- **No schema-attached validators on `@keel/db` tables.** The table value
...
1. **Add `zod` as a workspace dep** at the root (~50KB). Single version
   across all packages.
2. **`@keel/web`: add `validateBody<S>(schema, request)`** helper that
...
5. **Templates (`packages/create-keel`)** get one example controller
...
- **Form generation.** Generating HTML forms from Zod is a real thing
  (`zod-form`, custom) but it's an admin concern, not a Keel-wide one.
...
- Error shapes are machine-readable everywhere (Zod's `flatten()` /
  ...
  shape.
- Admin moves. Without it, admin stays on `@keel/orm` and Phase D
