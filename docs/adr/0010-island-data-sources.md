...
**The cache-header rule for the auto-exposed `GET /__keel/data/<name>` route** (binding on `keel().data()` and any future transport):
...
5. **The auto-route set no cache headers.** `keel().data()` returned `c.json(...)` with only a content-type — per-user JSON, heuristically cacheable. Fixed by the §3a scope + cache-header rule.
...
7. **No deadline on bind resolution.** A hung `/__keel/data/<name>` left its island `deferred` forever (contrast the 10s stream deadline). Fixed: bind resolution races a 10s timeout (`UI_ISLAND_DATA_TIMEOUT`) and routes to `onMountError`/`failed`.
8. **Known limitation (documented, not fixed):** `.data()` registered on a sub-app mounted under a `.route(prefix, …)` prefix gets its *route* prefixed but its `bind.href` still points at the root `/__keel/data/<name>` — register sources on the root app. A prefix-aware href is future work.
9. **Future seams noted, deliberately deferred:** a CSP nonce seam for the inline primer (no Keel serving path enforces a CSP today; `RECOMMENDED_CSP` in `@keel/web/harden` is opt-in), and generalizing `ObserveFn` to a `TriggerFn` so the hydration-strategy vocabulary can grow beyond `"load" | "visible"` (e.g. `"idle"`, `"media"`).
