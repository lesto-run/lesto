...
- **Status:** Accepted — **Increments 1 and 2 BUILT** (updated 2026-06-16, Wave 5 `ui-client#6`/`#7`).
  - **Increment 1 — the wire, proven end-to-end.** `defineIsland`/`hydrateDocumentIslands` ship; `render-page.tsx` emits the `<head>` client module tag (config-driven via `keel().client(...)`, per the 2026-06-11 amendment below); `@keel/assets` synthesizes the client entry from the `app/islands/` convention; `examples/blog` ships a live `ssr: true` data-bound island on `/posts` (the canonical-island proof, ADR 0012). The remainder-of-Increment-1 + the review bug list + ADR 0012's inversion landed as `docs/plans/island-data-hardening.md`.
  - **Increment 2 — estate convergence (the regression gate) DONE.** estate authors all three islands (`Account`, `LiveListing`, `DeferredPanel`) as one-`defineIsland`-per-file modules under `examples/estate/app/islands/`, the same convention `@keel/assets` synthesizes — so its client entry is the canonical synthesized shape, not a bespoke hand-list. The two island PATHS are converged: the `.page`/`defineIsland`/`hydrateDocumentIslands` path is CANONICAL; the Registry/`UiNode`/`renderPage`/`hydrateIslands`-array path is DEMOTED to the AI-/DB-content niche (its sole proof is the `@keel/e2e` fixture). Both emit the byte-identical `IslandMount` through the one shared `islandMount` author, so the island invariants live in exactly one emission site. `Registry.defineClient` typing is resolved by DECISION (it stays erased on purpose — the typed authoring path is `defineIsland`'s `IslandDef<P, D>`).
  - **Diagnostics (`ui-client#7`):** `RenderError` carries the thrown error (`detail`/`cause`) on `render_threw` and surfaces `validateProps` failures as `invalid_props`; `@keel/assets` refuses a malformed `app/islands/` module (`ASSETS_BAD_ISLAND_MODULE`) and a blown gzip budget (`ASSETS_BUDGET_EXCEEDED`), narrating per-artifact gzip sizes; the Registry warns on cross-namespace shadowing.
  - **Remaining:** Increment 3 (scaffold flip — `create-keel` emits `app/islands/` + `ui:{dialect:"preact"}`) and Increment 4 (dev tree-lint + compat-risk check; the rest of Increment 4 — inline-data tier, content-hashed entry, size heuristic, dialect-pair errors — already landed across `island-data-hardening` + `ui-client#7`).
