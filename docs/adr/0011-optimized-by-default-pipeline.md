...
- **Status:** Accepted — **Increments 1 and 2 BUILT** (updated 2026-06-16, Wave 5 `ui-client#6`/`#7`).
  - **Increment 1 — the wire, proven end-to-end.** `defineIsland`/`hydrateDocumentIslands` ship; `render-page.tsx` emits the `<head>` client module tag (config-driven via `lesto().client(...)`, per the 2026-06-11 amendment below); `@lesto/assets` synthesizes the client entry from the `app/islands/` convention; `examples/blog` ships a live `ssr: true` data-bound island on `/posts` (the canonical-island proof, ADR 0012). The remainder-of-Increment-1 + the review bug list + ADR 0012's inversion landed as `docs/plans/island-data-hardening.md`.
  - **Increment 2 — estate convergence (the regression gate) DONE.** estate authors all three islands (`Account`, `LiveListing`, `DeferredPanel`) as one-`defineIsland`-per-file modules under `examples/estate/app/islands/`, the same convention `@lesto/assets` synthesizes — so its client entry is the canonical synthesized shape, not a bespoke hand-list. The two island PATHS are converged: the `.page`/`defineIsland`/`hydrateDocumentIslands` path is CANONICAL; the Registry/`UiNode`/`renderPage`/`hydrateIslands`-array path is DEMOTED to the AI-/DB-content niche (its sole proof is the `@lesto/e2e` fixture). Both emit the byte-identical `IslandMount` through the one shared `islandMount` author, so the island invariants live in exactly one emission site. `Registry.defineClient` typing is resolved by DECISION (it stays erased on purpose — the typed authoring path is `defineIsland`'s `IslandDef<P, D>`).
  - **Diagnostics (`ui-client#7`):** `RenderError` carries the thrown error (`detail`/`cause`) on `render_threw` and surfaces `validateProps` failures as `invalid_props`; `@lesto/assets` refuses a malformed `app/islands/` module (`ASSETS_BAD_ISLAND_MODULE`) and a blown gzip budget (`ASSETS_BUDGET_EXCEEDED`), narrating per-artifact gzip sizes; the Registry warns on cross-namespace shadowing.
  - **Remaining:** Increment 3 (scaffold flip — `create-lesto` emits `app/islands/` + `ui:{dialect:"preact"}`) and Increment 4 (dev tree-lint + compat-risk check; the rest of Increment 4 — inline-data tier, content-hashed entry, size heuristic, dialect-pair errors — already landed across `island-data-hardening` + `ui-client#7`).
- **Date:** 2026-06-11 (status refreshed 2026-06-16)
- **Context:** over this session `examples/estate` was hand-tuned to a near-perfect Lighthouse — a Preact client (~11 KB vs React's ~118 KB), per-island code-splitting, island data via a parse-time primer (ADR 0010, no waterfall), minified/prod bundles, deferred non-blocking scripts, correct `<head>` + a `<main>` landmark. But it got all of it from **six bespoke estate-owned files** (`build-client.ts`, `build.ts`, `dev.ts`, `src/document.ts`, `src/production.ts`, `worker.ts`). A *new* Lesto app — the `create-lesto` scaffold, or the canonical `lesto() + .page` path (`examples/blog`) — gets **none of it**. The framework has the *capabilities* (in `@lesto/ui`) but not the *defaults*.
...
Every optimization exists in `@lesto/ui` as an injectable *capability*; the gap is **composition** — it lives in estate's six files, so the framework knows *how* but no framework path *does*. The fix is to own three seams.
...
A new `defineIsland(def)` in `@lesto/ui` wraps a `ClientComponentDef` and returns a React component usable directly in any `.page`/layout JSX (`<AccountIsland />`). When server-rendered it emits, **co-located in the stream**:

1. the marked shell — `<div data-lesto-island={id}>` holding the fallback (deferred) or the real output (`ssr: true`), exactly `buildIsland`'s contract, `id` from `useId()`;
2. an adjacent `<script type="application/json" data-lesto-island-mount>` carrying *its own* `IslandMount` JSON (through the existing `serializeManifest` escaping — one mount-script per island, not one page-wide array);
...
**Dialect is one config key, not four flags.** `lesto.config` grows `ui: { dialect: "preact" | "react" }` that atomically drives the client alias, the server `ServerRenderer`, the wrangler alias, and the dev server — so the matched pair (ADR 0008) is one decision, and the half-configured mismatch is unrepresentable. `lesto dev` runs the configured dialect, so **dev == prod** (closing the divergence estate has today).

### Seam 3 — The CLI owns the loop

`lesto build` runs `buildClient` (prod) into each static site's out dir when `app/islands/` exists; `lesto dev` runs it unminified on boot + a debounced watcher; `lesto deploy` composes `build`. estate's `build.ts`/`dev.ts`/`production.ts` *become* these commands.
...
| Dev-time **tree-lint** (missing `<main>`, skipped heading, CLS-risk fallback) | **Safe default** — Lesto renders walkable `UiNode`/island JSON, so structure is *mechanically auditable* — a check no React framework can do |
...
3. **Increment 3 — scaffold flip.** `create-lesto` emits `app/islands/` + a data-bound island + `ui:{dialect:"preact"}`. Every new app is born optimized.
