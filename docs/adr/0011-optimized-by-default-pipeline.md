...
- **Status:** Accepted — design. Increment 1 is **partially built**: the `@keel/ui` island wire (`defineIsland`/`hydrateDocumentIslands`) and the `@keel/assets` client-pipeline core are shipped and unit-tested, but the wire is **NOT yet proven end-to-end on a real app** — `render-page.tsx` does not yet emit the head module tag (still `islands: []`), `@keel/assets` is not yet CLI-invoked, and `examples/blog` has no island. So a `<…Island/>` in a `.page` today emits a shell + mount script that nothing hydrates. The blog proof, the render-page chrome, and the CLI wiring are the remainder of Increment 1; Increments 2–4 (estate convergence, scaffold flip, hardening) follow. **The remainder of Increment 1, the review's bug list, and ADR 0012's canonical-island inversion are sequenced as one executable plan: `docs/plans/island-data-hardening.md` (2026-06-11).**
- **Date:** 2026-06-11
