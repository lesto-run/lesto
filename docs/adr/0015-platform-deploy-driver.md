...
Volo generated a `wrangler.jsonc` (`@volo/cloudflare`'s `wranglerConfig`) and had
versioned-release machinery (`@volo/deploy`'s `shipRelease`/`rollback`, the local
`nodeReleaseStore`, and a real S3/R2 `remoteReleaseStore`) — but **nothing wired
them into a real deploy**: `volo deploy`'s dynamic-zone branch only *printed*
`"run volo serve (dynamic)"`, `wranglerConfig` had zero non-test callers, and
...
The question was *how* `volo deploy` should reach Cloudflare. Three options:
...
1. **An IaC framework (Alchemy / SST).** A heavyweight external dependency that
   ...
   and massive over-engineering for Volo's deliberately tiny resource surface (one
   Worker, one D1, static assets, two secrets). Rejected: it is the "zoo of
   external services" Volo exists to avoid, and it couples Volo's deploy identity
   to a third party's abstractions.
2. **A direct Cloudflare-API client (build our own).** Maximally in-house, and Volo
...
3. **The official `wrangler` CLI.** Always current (Cloudflare maintains it as the
   platform evolves), and the designed consumer of the `wrangler.jsonc` Volo
...
upload protocol and it moves — exactly the category where Volo already chooses a
thin driver (mail transport, object storage, OAuth). So:

- **Use `wrangler` (option 3) now, behind a Volo-owned `CloudflareDeployer`
...
`volo deploy --cloudflare` builds the static output (so `wrangler` uploads fresh
...
- **Deferred (follow-ups), not in this slice:** wiring `wranglerConfig` into the
  ...
  a CLI-convention example exercising `volo deploy --cloudflare` end-to-end (the
  ...
  *static* target — `volo deploy --release --bucket/--endpoint`, credentials from
