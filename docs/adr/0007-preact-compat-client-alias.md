...
> **Follow-up (2026-07-02, L-7b2ad4b2 / L-381d648b): the CLIENT no longer needs the `react-dom` shim.** Commit `0f2c627` moved React 19's resource-hint surface (`resources.ts`) off the isomorphic `@lesto/ui` barrel onto the `@lesto/ui/server` subpath, so the client entry no longer reaches bare `react-dom` — the `--preact` client bundle is byte-identical with `build-client.ts`'s `react-dom` alias entry removed, and that entry has been dropped. Both shim *files* remain: estate's SSR **worker** imports `@lesto/ui/server` (for `preactServerRenderer`), which still drags `resources.ts` (bare `react-dom`) and the page renderers (`react-dom/server`) into its graph, so `wrangler.jsonc` keeps aliasing both specifiers to the inert shims. The framework's default client path already dropped both from `@lesto/assets`' `PREACT_ALIAS` for the same reason. (`react-dom/server` is likewise dead in estate's *client* graph and could be dropped from `build-client.ts` too — a scoped follow-up.)
