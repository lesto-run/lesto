...
**Keel is the framework that makes a server-rendered, database-backed site feel like a
...
D1/Durable Objects as a database-substrate**. That is *Keel's exact thesis*, funded and
...
**What we ship:** a `@keel/platform` layer that, by default, emits `@view-transition`, injects a
...
> Result: a plain Keel page navigates **faster *and* smoother than most React SPAs**, with no
...
interface already exist — we fill them into a real pipeline. **New package: `@keel/assets`.**
...
- `@keel/storage` gains an **S3/R2 backend** and a **`url()`** (public + presigned) method — the
  two gaps the review found.
- `keel deploy` **uploads automatically**: incremental, changed-files-only, to any S3-compatible
...
The differentiator nobody else has — but today it's unwired (no `keel mcp` command, three
divergent MCP impls, `generate_ui` inert). We make it a five-minute demo:

- A real **`keel mcp`** binary/command; **collapse the three MCP implementations into one**
...
| Package | Change |
|---|---|
| **`@keel/assets`** (new) | `public/` + import-graph fingerprinting, manifest, `<Image>`/`<Asset>`, build-time variant generation, CDN-URL rewriting. |
| **`@keel/platform`** (new, or fold into runtime/loom) | `@view-transition` + speculation-rules emission, `view-transition-name` authoring, `rel=expect` injection, bfcache/prerender-safety helpers, default Service Worker recipe (precache assets + Navigation Preload for dynamic HTML + `Vary` correctness). |
| **`@keel/storage`** | Add **S3/R2 backend** + **`url()`** (public/presigned). |
| **`@keel/deploy`** | Fill the `uploader` seam: incremental S3-compatible uploader, immutable headers, CDN invalidation; a **first-class Cloudflare adapter** (R2 + Workers + 103 Early Hints) as flagship; align targets with Vite Environments. |
| **`@keel/web` / `@keel/runtime`** | **Binary/stream response bodies** (kill string-only `body`); full MIME table in `contentTypeOf`; emit `Cache-Control`/`immutable`; `fetchpriority`/`modulepreload` hints; Early Hints hook. |
| **build tier** | Vite 8/Rolldown replaces `bun build`; Environments + ModuleRunner; content plugin unchanged. |
| **`@keel/mcp` + content MCP** | One implementation; `keel mcp` bin; `generate_ui` wired; ops surface expanded. |
...
**Phase 2 — The asset pipeline, Bet II (≈3–4 wks).** `@keel/assets`: `public/` + import
fingerprinting, manifest, `<Image>` with the full perf stack, build-time variants. `@keel/storage`
S3/R2 backend + `url()`. `@keel/deploy` auto-uploader + immutable headers + invalidation. *Exit:
`keel deploy` fingerprints every asset, uploads changed-only to R2/S3, serves via CDN, and the Node
tier never streams an asset byte — with zero user-written upload script.*

**Phase 3 — The platform router, Bet I (≈2–3 wks).** `@keel/platform`: view transitions +
speculation rules + `rel=expect` + bfcache/prerender-safety + default SW recipe + `view-transition-name`
authoring. *Exit: a stock Keel app gets animated transitions (Chromium+Safari) and prerendered
...
`keel deploy --target cloudflare` puts assets on R2 + app on Workers + Early Hints.*

**Phase 5 — Agent control plane, Bet IV (≈2–3 wks).** `keel mcp`; one MCP impl; `generate_ui`
...
- **MCP triplication** (~1,700 lines across `@keel/mcp`, `content-core/mcp*.ts`, `content-mcp`) —
  one impl.
...
- **Dead code**: the 14-event build taxonomy (`events.ts`), the unreachable workflow auto-filter
  path, duplicated content-search/embeddings primitives (`@keel/content-shared` already exports
...
1. `bunx create-keel shop && cd shop && keel dev` — boots in <2s (Rolldown HMR).
...
5. `keel deploy --target cloudflare` — assets fingerprinted to R2 behind the CDN, app on Workers,
