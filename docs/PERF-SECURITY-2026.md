# Keel — Performance & Security Landscape (mid-2026) and Native-Tools Plan
...
> the PHP incumbents) + a code recon of Keel's current state.
>
...
1. **Response body: `string` → `ReadableStream | Buffer | string`** (and let `applyResponse` send a
   bodiless 304). Today `KeelResponse.body` is a string and `applyResponse` is `writeHead + end(body)`
   ...
   perf story.
2. **A middleware pipeline + `AsyncLocalStorage` request context.** Keel has no interception point,
   so the already-built `@keel/csrf`, `@keel/cors`, `@keel/ratelimit` are **dead code** — nothing
...
### Server / transport
| Capability | Who ships it | Native primitive | Keel | Impact/Effort |
|---|---|---|---|---|
...
### React SSR
| Capability | Native primitive | Stable? / who | Keel | Notes |
|---|---|---|---|---|
...
| Layer | Capability | Native primitive | Keel | Impact/Effort |
|---|---|---|---|---|
...
| Server | data cache + `use cache`/cacheLife | memoizing wrapper over `@keel/cache` keyed by args | ✗ | High/L |
| Server | ISR / on-demand revalidate | revalidation store (ts + tag index) + bg regen on `@keel/sites` | ✗ | High/L |
| Server | full-page + fragment cache | render memoized in `@keel/cache`, tag-indexed | ✗ | High/L |
...
| Capability | Native primitive | Keel | Impact/Effort |
|---|---|---|---|
...
| Wire csrf into the pipeline (default-on for state-changing methods) | existing `@keel/csrf` + `node:crypto`, session-bound | ◐ (exists, unwired) | High/S once pipeline exists |
...
structurally require a DB substrate** — which Next/Remix/TanStack lack and Keel uniquely has. Supabase
acquired Triplit; the DB vendors are absorbing local-first. **Keel can ship it natively because it
owns the substrate.**

| Piece | Native primitive | Maturity | Keel fit |
|---|---|---|---|
...
| Client store | in-memory + incremental view maintenance (TanStack DB style); opt-in **SQLite-wasm + OPFS** (Chrome 102+/FF 111+/Safari 16.4+, Safari<17 VFS caveat) | TanStack DB beta→1.0; OPFS broadly avail | mirrors Keel's local SQLite |
| Writes | optimistic apply → POST through existing ORM/queue → server-authoritative reconcile | proven (Zero/TanStack) | reuses Keel batteries; no CRDT needed in v0 |
...
`@keel/runtime`, `@keel/ui`, `@keel/cache` (each 100% covered; full suite green). What shipped:
- ✅ **HTTP cache headers** (`@keel/runtime/http-cache.ts`): `immutable` for content-hashed assets / `no-cache` for HTML via `cacheControl()` + `hasContentHash()`; content-hash **ETag → bodiless 304** (`etagFor`/`etagMatches`/`respondNotModified`, ETag stripped of `Content-Length`); SWR/`stale-if-error` directives; `Vary`. `KeelResponse.body` stays a string (304 bypasses `applyResponse`).
- ✅ **React resource preloading** (`@keel/ui/resources.ts`): `preload`/`preinit`/`preinitModule`/`preconnect`/`prefetchDNS` + `lcpImage` (`fetchpriority=high`) + `modulePreload` — hoist into the buffered `<head>` (verified empirically).
- ✅ **Document-metadata** (`@keel/ui/metadata.ts`): title/meta/link helpers + `dedupeMetadata` (charset promoted to front for the first-1024-bytes rule).
...
- ✅ **bfcache** (`@keel/ui/bfcache.ts`): `observePageLifecycle` uses only `pagehide`/`pageshow`/`visibilitychange`.
- ✅ **Single-flight coalescing** (`@keel/cache` `remember()`): `Map<key, InFlight>` with a per-lead token so a `delete`/`clear` mid-flight wins the race (no stampede resurrection).
- ✅ **`createRoot` → `hydrateRoot`, made opt-in per island** (`ssr?` flag on `ClientComponentDef`, carried on the `IslandMount` wire). Keel islands server-render only a *fallback* by design (auth-aware static), so a blanket swap would mismatch; `ssr:true` makes the server render the real component and the client `hydrateRoot`s it. **Pitfall fixed:** the page must be serialized with the new `renderPageMarkup(page)` (uses `renderToString` when any island is `ssr:true` — `renderToStaticMarkup` strips the `<!-- -->` text-segment markers hydration needs).
...
- ✅ **B. Middleware pipeline + `AsyncLocalStorage` request context — SHIPPED.** Per-request ALS context in `@keel/web` (`runWithContext`/`currentContext()`; placed there to avoid a circular dep — runtime establishes, controllers read); strictly per-request via `AsyncLocalStorage.run` (no cross-request leak — verified with interleaved + concurrent-socket tests). Onion middleware pipeline (`Middleware`/`runPipeline`; `AppConfig.middleware`); `secureStack()` in `@keel/kernel` composing cors + rate-limit (safe defaults) + **opt-in** CSRF. `trust-proxy` in `@keel/runtime` (configurable; derives client ip/proto, off by default; spoofing-documented); `requestId` minted per request and added to the access log. `@keel/csrf`/`@keel/cors`/`@keel/ratelimit` now ship mountable middleware. **Backward-compat:** no-middleware apps unchanged; **CSRF never auto-on** (the estate tokenless sign-in still works). 6 packages 100% covered. **Follow-up:** the `@keel/cloudflare` edge handler must wrap dispatch in `runWithContext` + route through `applyResponse` to inherit the context + hardening (tracked in the server-streaming CVE review; owned by the edge track).
...
- ✅ **Streaming SSR — SHIPPED.** `@keel/ui` `renderPageStream(page)` → `ReadableStream` via React 19.2 `renderToReadableStream` (shell-first, `<Suspense>` progressive reveal; carries the `<!-- -->` hydration markers ssr:true islands need), plus `renderPageStreamToString(page)` — the buffered `allReady` exit for crawlers/SEO/SSG that **fails loudly (`UI_STREAM_INCOMPLETE`) rather than serving a partial page** when a boundary errors. `Controller.streamTree()` returns a stream-bodied response (flows through the 1.B pipeline unchanged; ETag skips streams). `@keel/sites` prerender stays buffered (untouched). Buffered `renderPageMarkup`/`renderTree` unchanged. ui + web 100% covered; estate (incl. hydration) unchanged. *Remaining for full streaming value: `use()`/Suspense data-loading conventions in app code.*
...
- **ISR/SWR** on `@keel/sites` (static-with-background-regen).
...
### Tier 4 — the differentiator: `keel.live(query)`
...
substrate, so they can only *consume* an external sync service; Keel can make `live()` a method on
...
Keel's path to "most performant + secure on native tools" is: ride React 19's native streaming +
...
`keel.live()` sync engine no app framework can coherently match.
