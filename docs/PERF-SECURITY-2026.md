# Lesto — Performance & Security Landscape (mid-2026) and Native-Tools Plan
...
> the PHP incumbents) + a code recon of Lesto's current state.
>
...
1. **Response body: `string` → `ReadableStream | Buffer | string`** (and let `applyResponse` send a
   bodiless 304). Today `LestoResponse.body` is a string and `applyResponse` is `writeHead + end(body)`
   ...
   perf story.
2. **A middleware pipeline + `AsyncLocalStorage` request context.** Lesto has no interception point,
   so the already-built `@lesto/csrf`, `@lesto/cors`, `@lesto/ratelimit` are **dead code** — nothing
...
### Server / transport
| Capability | Who ships it | Native primitive | Lesto | Impact/Effort |
|---|---|---|---|---|
...
### React SSR
| Capability | Native primitive | Stable? / who | Lesto | Notes |
|---|---|---|---|---|
...
| Layer | Capability | Native primitive | Lesto | Impact/Effort |
|---|---|---|---|---|
...
| Server | data cache + `use cache`/cacheLife | memoizing wrapper over `@lesto/cache` keyed by args | ✗ | High/L |
| Server | ISR / on-demand revalidate | revalidation store (ts + tag index) + bg regen on `@lesto/sites` | ✗ | High/L |
| Server | full-page + fragment cache | render memoized in `@lesto/cache`, tag-indexed | ✗ | High/L |
...
| Capability | Native primitive | Lesto | Impact/Effort |
|---|---|---|---|
...
| Wire csrf into the pipeline (default-on for state-changing methods) | existing `@lesto/csrf` + `node:crypto`, session-bound | ◐ (exists, unwired) | High/S once pipeline exists |
...
structurally require a DB substrate** — which Next/Remix/TanStack lack and Lesto uniquely has. Supabase
acquired Triplit; the DB vendors are absorbing local-first. **Lesto can ship it natively because it
owns the substrate.**

| Piece | Native primitive | Maturity | Lesto fit |
|---|---|---|---|
...
| Client store | in-memory + incremental view maintenance (TanStack DB style); opt-in **SQLite-wasm + OPFS** (Chrome 102+/FF 111+/Safari 16.4+, Safari<17 VFS caveat) | TanStack DB beta→1.0; OPFS broadly avail | mirrors Lesto's local SQLite |
| Writes | optimistic apply → POST through existing ORM/queue → server-authoritative reconcile | proven (Zero/TanStack) | reuses Lesto batteries; no CRDT needed in v0 |
...
`@lesto/runtime`, `@lesto/ui`, `@lesto/cache` (each 100% covered; full suite green). What shipped:
- ✅ **HTTP cache headers** (`@lesto/runtime/http-cache.ts`): `immutable` for content-hashed assets / `no-cache` for HTML via `cacheControl()` + `hasContentHash()`; content-hash **ETag → bodiless 304** (`etagFor`/`etagMatches`/`respondNotModified`, ETag stripped of `Content-Length`); SWR/`stale-if-error` directives; `Vary`. `LestoResponse.body` stays a string (304 bypasses `applyResponse`).
- ✅ **React resource preloading** (`@lesto/ui/resources.ts`): `preload`/`preinit`/`preinitModule`/`preconnect`/`prefetchDNS` + `lcpImage` (`fetchpriority=high`) + `modulePreload` — hoist into the buffered `<head>` (verified empirically).
- ✅ **Document-metadata** (`@lesto/ui/metadata.ts`): title/meta/link helpers + `dedupeMetadata` (charset promoted to front for the first-1024-bytes rule).
...
- ✅ **bfcache** (`@lesto/ui/bfcache.ts`): `observePageLifecycle` uses only `pagehide`/`pageshow`/`visibilitychange`.
- ✅ **Single-flight coalescing** (`@lesto/cache` `remember()`): `Map<key, InFlight>` with a per-lead token so a `delete`/`clear` mid-flight wins the race (no stampede resurrection).
- ✅ **`createRoot` → `hydrateRoot`, made opt-in per island** (`ssr?` flag on `ClientComponentDef`, carried on the `IslandMount` wire). Lesto islands server-render only a *fallback* by design (auth-aware static), so a blanket swap would mismatch; `ssr:true` makes the server render the real component and the client `hydrateRoot`s it. **Pitfall fixed:** the page must be serialized with the new `renderPageMarkup(page)` (uses `renderToString` when any island is `ssr:true` — `renderToStaticMarkup` strips the `<!-- -->` text-segment markers hydration needs).
...
- ✅ **A. Response body widened — SHIPPED.** `LestoResponse<B extends LestoBody = string>` where `LestoBody = string | Uint8Array | ReadableStream` (a `string` default keeps every existing consumer non-breaking). `applyResponse` writes all three arms (string→end, Uint8Array→Buffer, ReadableStream→`Readable.fromWeb().pipe` with source/destination error handling that destroys the socket, never crashes); ETag/304 skips streams (can't hash without draining). Binary-safe static serving: `nodeStaticReader` reads bytes, `contentTypeOf`/`isBinaryType` cover image/font/media/wasm, body kind reconciled to extension. `Controller.bytes()` helper added. Cloudflare adapter passes the body straight to a Web `Response`. **Verified:** the real server returns a PNG byte-for-byte (incl. a `0xFF` byte) + a streamed body; ws:typecheck + ws:test green (2688), web/runtime/cloudflare 100% covered, examples untouched. This unblocks streaming SSR + compression + binary + conditional GET.
- ✅ **B. Middleware pipeline + `AsyncLocalStorage` request context — SHIPPED.** Per-request ALS context in `@lesto/web` (`runWithContext`/`currentContext()`; placed there to avoid a circular dep — runtime establishes, controllers read); strictly per-request via `AsyncLocalStorage.run` (no cross-request leak — verified with interleaved + concurrent-socket tests). Onion middleware pipeline (`Middleware`/`runPipeline`; `AppConfig.middleware`); `secureStack()` in `@lesto/kernel` composing cors + rate-limit (safe defaults) + **opt-in** CSRF. `trust-proxy` in `@lesto/runtime` (configurable; derives client ip/proto, off by default; spoofing-documented); `requestId` minted per request and added to the access log. `@lesto/csrf`/`@lesto/cors`/`@lesto/ratelimit` now ship mountable middleware. **Backward-compat:** no-middleware apps unchanged; **CSRF never auto-on** (the estate tokenless sign-in still works). 6 packages 100% covered. **Follow-up:** the `@lesto/cloudflare` edge handler must wrap dispatch in `runWithContext` + route through `applyResponse` to inherit the context + hardening (tracked in the server-streaming CVE review; owned by the edge track).
...
- ✅ **Streaming SSR — SHIPPED.** `@lesto/ui` `renderPageStream(page)` → `ReadableStream` via React 19.2 `renderToReadableStream` (shell-first, `<Suspense>` progressive reveal; carries the `<!-- -->` hydration markers ssr:true islands need), plus `renderPageStreamToString(page)` — the buffered `allReady` exit for crawlers/SEO/SSG that **fails loudly (`UI_STREAM_INCOMPLETE`) rather than serving a partial page** when a boundary errors. `Controller.streamTree()` returns a stream-bodied response (flows through the 1.B pipeline unchanged; ETag skips streams). `@lesto/sites` prerender stays buffered (untouched). Buffered `renderPageMarkup`/`renderTree` unchanged. ui + web 100% covered; estate (incl. hydration) unchanged. *Remaining for full streaming value: `use()`/Suspense data-loading conventions in app code.*
...
- **ISR/SWR** on `@lesto/sites` (static-with-background-regen).
...
### Tier 4 — the differentiator: `lesto.live(query)`
...
substrate, so they can only *consume* an external sync service; Lesto can make `live()` a method on
...
Lesto's path to "most performant + secure on native tools" is: ride React 19's native streaming +
...
`lesto.live()` sync engine no app framework can coherently match.
