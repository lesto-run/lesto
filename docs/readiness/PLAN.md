...
| 0.4 | **Stronger default `secureStack`** in the scaffold — add `rateLimit` + a CSRF token to the generated app (primitives already compose in safe order) | S | `create-keel/templates.ts`, `kernel/secure-stack.ts:85` | new apps ship rate-limit + CSRF by default |
...
| 1.2 | **Flip the interface** in `@keel/db`: `exec`, `run`, `get`, `all` → `Promise`. Update the query builder so terminals `await` the driver. Keep `defineTable`/columns/conditions/DDL untouched (pure values). | M |
...
| 1.4 | **Postgres adapter** — new `@keel/pg` (or `@keel/db/pg`): a `node-postgres` (`pg`) `Pool` implementing the async `SqlDatabase`; `?`→`$1` placeholder translation; type/row mapping. | L |
| 1.5 | **Migrate `@keel/migrate`** to `await` its DDL/bookkeeping (it owns `exec` + the versions table). | M |
...
Today `keel deploy` is a planner + non-atomic file copy; the dynamic tier just
prints `run keel serve`.
...
| 3.3 | **Rollback command** — `keel rollback` repoints `current` to the previous version. | S |
| 3.4 | **Dynamic-tier deploy** — beyond printing `keel serve`: a container recipe (Dockerfile already exists) or a process-target adapter, with the same health-gate. | L |
...
| Step | Work | Effort |
|------|------|--------|
| 4.1 | **OTel exporter adapter** for `@keel/observability` (it ships in-memory only today) — OTLP out, behind the existing exporter interface. | M |
...
| 5.3 | **Wire `@keel/rbac` into a request-path authorization middleware** — it's pure logic wired into zero request paths today; mount it in `secureStack`. | M |
