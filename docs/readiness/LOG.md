...
| 2026-06-10 (run 2, post async-merge) | 6.5/10 | +1.0 (from 5.5) | deploy is a non-atomic file copy + observability orphaned (no traces/metrics, `/readyz` lies) — structural | Reformat `@volo/db` (committed `oxfmt` regression) (S) |
| 2026-06-16 (post Waves 0–5) | 6.8/10 | +0.3 (from 6.5) | `volo deploy` is still a file copy (never invokes `wrangler deploy`; `remoteReleaseStore` unwired) + secure defaults opt-in, not kernel-enforced — structural | Add `server.maxConnections` cap (`server.ts`, S) |
...
(sync ORM deleted; async `@volo/db` + real `@volo/pg` adapter, transactional cross-process-locked
...
**Three caps:** (1) `volo deploy` is verified to be a file copy — `remoteReleaseStore` has zero refs
...
Fruit ceiling: **~7.4**. Judge's call: **PIVOT** to a structural phase — real `volo deploy`
...
ceilings dominate: async/Postgres unbuilt, security pipeline unwired, `volo deploy` is a file copy.

## 2026-06-09 (run 2) — 5.5/10

After Tier 0 (`a1b84ac`), Keystone 1 binary/stream body (`b37438f`), and Tier 1.B pipeline +
ALS context + Tier 2 streaming SSR (`d730810`), plus the parallel track's `@volo/identity` and
...
structural phase** — the six biggest blockers (async/Postgres, real `volo deploy`, CF-edge hardening
