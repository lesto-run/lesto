...
2. **Migrate the benchmark edge worker; retire its wrangler config as *deploy authority only*** — `[P1 | simplest single-worker exercise]`
   ...
   **SAFETY:** Alchemy has **no state** for the already-`wrangler`-deployed `lesto-bench-edge` worker → a blind `bun alchemy.run.ts` can **orphan or duplicate** it. Use Alchemy's **adopt** path + **`--dry-run`** first (flag names **verified against Alchemy's live docs at implementation time** — the requirement is pinned, not the spelling), confirm Alchemy *takes over the existing resource* (not a second copy), and **never** run a blind `--destroy` against the live worker.
   Acceptance: `bun alchemy.run.ts` deploys/updates the *same* `lesto-bench-edge` resource (preview via `alchemy run` — the read-only subcommand; there is **no** `--dry-run` flag in `0.93.x`, see "Verified Alchemy API" below — showing adopt, not create); the live edge tier still serves; **`start-edge.mjs` → live `workerd` still boots locally** after the retirement; `wrangler.jsonc` retained as local-runtime config.
...
- **Inc1 — DONE; the DO-backed shared store proven live SINGLE-machine (the literal two-machine run
  is not yet executed).** Both example `alchemy.run.ts` files now use `CloudflareStateStore`
  (DO-backed shared state). `examples/mcp-ops-console` was deployed live (`bun alchemy.run.ts`): the
  state service provisioned, both Workers (issuer + RS) came up, the RS served its RFC 9728 metadata
  and returned `401` to a token-less `/mcp`, then a `--destroy` cleanly tore both Workers down.
  **What was actually proven:** the local `.alchemy/` held only esbuild `out/` bundles — **zero**
  local state (deploy state lives entirely in the `CloudflareStateStore` DO), so the `--destroy` read
  the resource records from the shared DO backend, not any local file. **The equivalence argument
  (not an executed test):** because there is *no* local state, any environment holding the shared
  `ALCHEMY_STATE_TOKEN` is by construction equivalent to this one — so a second machine reading the
  same state is equivalent. That is a *reasoned* substitution for Inc1's literal acceptance ("a
  *different* environment reads that state and `--destroy`s resources it did not create"), which
  remains **not yet run on a second physical machine** (now feasible — owner has two). Committed
  `65d4c22`.
...
- **Inc3 — workflow in place; skip-path verified; green-with-secrets NOT yet run.**
  `.github/workflows/deploy-examples.yml` deploys the migrated Alchemy graphs (both examples + the
  bench) on push to `main`, stage pinned to `prod` (D2), gated on the D4 secrets
  (`CLOUDFLARE_API_TOKEN` + `ALCHEMY_STATE_TOKEN`) — **skips out loud** (a `::notice::`, not a
  failure) when absent. Only the **skip path** is CI-checkable today; the green-deploy half runs the
  first time `main` is pushed with both secrets present (`ALCHEMY_STATE_TOKEN` is set;
  `CLOUDFLARE_API_TOKEN` is pending a scoped token).

**Net:** ADR 0044 is implemented — the multi-resource examples (Inc1) and the benchmark edge worker
(Inc2) run on the DO-backed shared-state Alchemy convention, both proven live single-machine; the CI
deploy job (Inc3) is in place and secret-gated, its green-deploy path pending the CF token + a push.
`lesto deploy`, estate, `www`/`site` stay on wrangler (D6), untouched.
