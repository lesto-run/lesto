...
Packages: `@keel/auth`, `@keel/identity`, `@keel/csrf`, `@keel/cors`, `@keel/ratelimit`,
`@keel/authz`, `@keel/rbac`, `@keel/flags`.
...
7. **authz/rbac consolidation** — `[Wave 5 | P1]`
   Fold `@keel/rbac`'s wildcard permissions + cycle-safe inheritance into `@keel/authz`'s `definePolicy`; delete or attic `@keel/rbac`. One authorization story before the API freezes.
