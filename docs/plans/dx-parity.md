...
- **Design decision to make FIRST (gates the rest):** the route registry (`RegisteredRoutes`) lives in `@lesto/ui` today because `<Link>` consumes it. A typed `route()` builder also needs the registry **plus** `PathParams` + param-substitution, which live in `@lesto/client`. Decide where the registry belongs — keep it in `@lesto/ui` with a local param type, or **lower it into a shared low package** (e.g. `@lesto/router`) that both `<Link>` (href) and `route()` (pattern + params) read — so the two typed-nav surfaces share ONE source of truth instead of drifting. Do NOT build `route()` before this is settled; rushing it risks a cross-package coupling that's expensive to undo.
