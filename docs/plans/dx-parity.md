...
> **Reassessment (2026-06-20) — two things delivered, the rest descoped.** The original review's complaint ("a typo'd link is a runtime 404, not a `tsc` error") is addressed two ways:
> 1. **`route()` (shipped, `70fbb73`) — opt-in, any app.** `route("/about")` and `route("/blog/:id", { id })` each error on a typo'd pattern or a wrong/missing param and return a `string` for `<Link href>`. So the complaint is answered *for any link you opt into checking via `route()`* — but a bare `<Link href="/typo">` still compiles by design (the lenient `(string & {})` escape).
> 2. **`<StrictLink>` (this commit) — strict by default, fully-file-routed apps.** A re-typed `Link` whose `href` is `StrictRouteHref` (the known routes with NO escape), so a typo'd bare `href` is a `tsc` error with no `route()` wrapper. Runtime-identical to `Link`, zero new code. Sound only when the codegen registry is the complete route set — a MIXED app (with code-first `.page()` routes) keeps lenient `<Link>`.
>
> **Descoped (retained below as design-if-ever-justified, not scheduled):** strict BY DEFAULT for *mixed* apps needs **code-first route capture**, a large refactor — `Lesto` is a concrete class whose `.page()` returns `this`, so capturing code-first route types means a generic `Lesto<Routes>` threaded through every builder method and every `: Lesto` consumer (~20 sites). The marginal gain over `route()` + `<StrictLink>` does not justify it. `navigate()` typing has no public API to attach to (a separate feature), and the prefix-desync is a documented edge no app hits.
...
**Why (verified):** the scaffold writes 9 files and exits with "now run bun install" — no install, no `git init`, no prompts, no Tailwind/lint/test wiring, no `AGENTS.md`/`CLAUDE.md`, a home page in raw `createElement` (not JSX), and **no `app/routes/` example** so the headline file-routing is invisible on day 1 (`packages/create-lesto/src/scaffold.ts:104-114`, `templates.ts:192-200`). Worse, deps pin `^0.1.0` against an **unpublished** registry (`index.ts:42-45`) — `npm create lesto` fails to install without the internal `--local` flag (verifier confirmed npm 404). `create-next-app` runs a wizard, installs, inits git, scaffolds App Router + an `AGENTS.md`.
