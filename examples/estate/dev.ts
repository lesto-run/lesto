 ...
 *     served HTML gains a script that polls `/__lesto/version` and reloads when
...
// secrets) unless the operator set LESTO_AUTH_SECRET. The deployed Worker never
 ...
 * and save again.
 *
 * `LESTO_PREACT=1` opts the CLIENT bundle into Preact's compat layer (see
...
/**
 * The inline script injected into dev HTML: poll `/__lesto/version` and reload
 * the page when the number changes. Polling, deliberately — `LestoResponse.body`
...
const POLL_SCRIPT = `<script>(() => {
  ...
  setInterval(async () => {
    try {
      const { version } = await (await fetch("/__lesto/version")).json();
...
/**
 * Wrap the dispatcher with the dev reload loop: answer `/__lesto/version` with
...
function withDevReload(dispatch: Dispatch): Dispatch {
  return async (method, path, options) => {
    if (method === "GET" && path === "/__lesto/version") {
      return {
        status: 200,
