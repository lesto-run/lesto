...
import type { CurrentSpan } from "@keel/observability";
...
async function main(): Promise<void> {
  ...
  // unless `KEEL_OTLP_URL` is set (the two-env-var setup, see README). The
  ...
  //
  //   KEEL_OTLP_URL=http://localhost:4318/v1/traces  bun run serve.ts
  //
  // This is estate-as-the-OTLP-reference: the SAME `tracesFromEnv` + seam wiring
  // a production Keel app uses, dogfooded on a real app.
  const traces = tracesFromEnv(process.env, {
    currentSpan: currentRequestSpan as CurrentSpan,
  });

  if (traces !== undefined) {
    console.log("OTLP tracing on → spans flush to KEEL_OTLP_URL");
  }
