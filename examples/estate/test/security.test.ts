 ...
 * `@lesto/ui`'s `renderMetadata`, island props via `serializeScriptJson`) is
 * pinned at 100% coverage in `@lesto/ui` and `@lesto/web` themselves.
 */

import { describe, expect, it } from "vitest";

import { SESSION_COOKIE } from "@lesto/identity";

import { buildApp } from "../src/app";
import { DEFAULT_DEMO } from "../src/identity";

// ---------------------------------------------------------------------------
// Session cookie attributes — pinned at the @lesto/identity boundary; estate's
// `__Host-` prefix discipline now lives there. A regression in the prefix or
// attribute set would show up in @lesto/identity's tests, not here.
// ---------------------------------------------------------------------------
...
describe("the /mls POST handlers", () => {
  ...
  it("accept a same-origin sign-in with valid demo credentials, and set the session cookie", async () => {
    ...
    expect(res.headers["Set-Cookie"]).toContain("__Host-lesto_session=");
...
// `lesto().data()` answers it with `Cache-Control: private, no-store`. This is
// the live launch-hardening surface: a missing header here is a session leak
// waiting for a CDN.
// ---------------------------------------------------------------------------

describe("the /__lesto/data/session route", () => {
  it("is no-store — the per-user session JSON is never written to a shared cache", async () => {
    const app = await buildApp();

    const res = await app.handle("GET", "/__lesto/data/session");
