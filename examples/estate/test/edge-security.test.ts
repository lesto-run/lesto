...
describe("edgeSecret — fail closed (blocker #1)", () => {
  ...
  // so they manage LESTO_DEMO / SESSION_SECRET themselves and restore after.
  let savedDemo: string | undefined;
  let savedSecret: string | undefined;

  beforeEach(() => {
    savedDemo = process.env["LESTO_DEMO"];
    savedSecret = process.env["SESSION_SECRET"];
  });

  afterEach(() => {
    if (savedDemo === undefined) delete process.env["LESTO_DEMO"];
    else process.env["LESTO_DEMO"] = savedDemo;
  ...
  it("uses the committed demo fallback ONLY under an explicit LESTO_DEMO=1 binding", () => {
    delete process.env["LESTO_DEMO"];
    delete process.env["SESSION_SECRET"];

    expect(isDemoMode({ LESTO_DEMO: "1" })).toBe(true);
    expect(edgeSecret({ LESTO_DEMO: "1" })).toMatch(/^estate-demo-edge-secret/);

    // A non-"1" value is NOT demo mode.
    expect(isDemoMode({ LESTO_DEMO: "true" })).toBe(false);
    expect(() => edgeSecret({ LESTO_DEMO: "true" })).toThrow();
  });
  ...
  it("reads LESTO_DEMO from process.env when no env binding carries it", () => {
    delete process.env["SESSION_SECRET"];
