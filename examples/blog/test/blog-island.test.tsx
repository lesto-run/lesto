 ...
 *
 * `examples/blog` is the canonical `lesto()+.page` app. This proves the CANONICAL
 ...
 *
 * No bespoke scripts: the page is `lesto()+.page` + one island file (review F8 —
...
const SEEDS = [
  { title: "Hello, Lesto", body: "First post body." },
...
describe("blog /posts — the canonical island", () => {
  it("streams the island's real markup with inline data, a mount script, no primer, the head tag", async () => {
    ...
    try {
      ...
      expect(html).toContain("data-lesto-island-mount");
  ...
  it("answers /__lesto/data/reactions with the shared cache header", async () => {
    const { app, close } = await bootBlog();
