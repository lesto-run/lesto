 ...
 * boots the very same app behind a real node:http server (`@lesto/runtime`'s
 ...
 *
 * The database is opened with `@lesto/runtime`'s `openSqlite`: under Node it
 ...
 *
 * OTLP TRACING (the two-env-var setup): `lesto serve`/`dev` and the estate
 * reference turn tracing on with one env var — set `LESTO_OTLP_URL` to a
 * collector's trace endpoint (e.g. `http://localhost:4318/v1/traces`), with
 * optional `LESTO_OTLP_SERVICE` (the `service.name`, default `lesto`) and
 * `LESTO_OTLP_HEADERS` (comma-separated `key=value`). Absent, tracing is off
...
async function main(): Promise<void> {
  ...
  // wraps the same handle as a typed @lesto/db for controllers + seeds.
