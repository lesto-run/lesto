/**
 * The BEHAVIORAL half of `serve.ts`: does the i18n app actually BOOT and SERVE
 ...
 * server that answers a real `fetch()`. This does, and it confirms the LOCALIZED
 * page renders end-to-end (a French greeting off a real HTTP round-trip).
 *
 ...
 * response, then SIGTERM and await a clean exit. `DB_PATH=:memory:` is harmless
 * here — serve.ts opens a throwaway in-memory handle regardless (i18n persists
 * nothing) — but is passed for parity with the gallery's other smoke tests.
 *
...
describe("@lesto/i18n example — serve.ts boots over real HTTP", () => {
  it("serves a localized GET /fr over a real socket and exits 0 on SIGTERM", async () => {
    ...
    // the French catalog + interpolation must render over the socket.
    const res = await fetch(`${base}/fr?name=Ada`);
