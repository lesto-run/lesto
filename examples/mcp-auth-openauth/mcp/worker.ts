/**
 * The Cloudflare Worker entry for the Lesto MCP Resource Server (Phase 4b).
 *
 * The SAME governed app as the Node path (`./governance.ts`), on the edge: no kernel, no
 * sqlite — a pure `@lesto/web` app whose `handle` `@lesto/cloudflare`'s `toFetchHandler` turns
 * into a Worker `fetch(Request) => Response`, running the exact transport-neutral hardening the
 * Node `serve` applies. `node:crypto`'s `createHash` (the MCP tool-input hash in `@lesto/mcp`)
 * runs under `nodejs_compat`; nothing else here is node-only.
 *
 ...
 *   - the JWKS fetch rides the `ISSUER` SERVICE BINDING when present. The issuer is another
 *     Worker on the same account, and a `workers.dev → workers.dev` subrequest is refused (CF
 *     error 1042) — so the RS reaches the issuer's JWKS through a service binding, not its public
 *     url. Against a real external IdP (a different origin) the binding is absent and the global
 *     `fetch` is used, exactly as a production RS would.
 */
...
export function buildHandler(
  ...
  const governed = buildGovernedApi({
    ...
    allowedOrigins: [],
    rolesOf: demoRolesOf,
    // Same-account issuer Worker → fetch its JWKS through the service binding (CF 1042 blocks
    // the public-url subrequest). Absent (real external IdP) → governance uses global `fetch`.
