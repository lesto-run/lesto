/**
 * The BEHAVIORAL half of `serve.ts`: does the hosted leg actually BOOT and
 * SERVE over a real socket? The other tests in this example drive the app
 * in-process (`app.handle` / `toFetchHandler`) — none proves the `serve.ts`
 * entry spawns a `node:http` server that answers a real `fetch()`. This does.
 *
 ...
 * which is exactly how the CLI e2e drives a `serve`-style server.
 */
...
describe("@lesto/webhooks example — serve.ts boots over real HTTP", () => {
  it("serves GET /received over a real socket and exits 0 on SIGTERM", async () => {
    ...
    // The receiver's inbox route: 200 with an empty log on a fresh process —
    // a real HTTP round-trip, not the in-process `handle` the other tests use.
    const res = await fetch(`${base}/received`);
    ...
    // serveWithGracefulShutdown drains (incl. `worker.stop()`) then exit(0) on
    // SIGTERM — a non-zero code (or a raw signal death) means teardown regressed.
