/**
 * The BEHAVIORAL half of `serve.ts`: does the cache app actually BOOT and SERVE
 ...
 * response, then SIGTERM and await a clean exit. `DB_PATH=:memory:` keeps the
 * smoke run off disk (serve.ts otherwise writes `./cache.db`); persistence is
 * proven by the in-process "warm key across a restart" test, not here.
 *
...
describe("@lesto/cache example — serve.ts boots over real HTTP", () => {
  it("serves a read-through GET /reports/:id over a real socket and exits 0 on SIGTERM", async () => {
    child = spawn("bun", ["run", "serve.ts"], {
      cwd: exampleDir,
      env: { ...process.env, PORT: "0", DB_PATH: ":memory:" },
    ...
    // A cache MISS computes the report from the origin: value is a deterministic
    // function of the id (`id.length * 100`), so "smoke" (5 chars) => 500. A real
    // HTTP round-trip, not the in-process `handle` the other tests use.
    const res = await fetch(`${base}/reports/smoke`);

    expect(res.status).toBe(200);

    const report = (await res.json()) as { id: string; value: number; generatedAt: number };

    expect(report.id).toBe("smoke");
    expect(report.value).toBe(500);
