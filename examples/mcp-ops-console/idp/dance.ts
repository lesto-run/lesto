 ...
 * providers (./issuer.ts). A real client/agent runs exactly this; here a small cookie-aware
 * redirect follower stands in for the browser.
 */
...
/**
 * Run the dance and return an access token for `provider` (`sre` | `oncall` | `viewer`), minted
 * for `clientID` (default {@link CLIENT_ID}; pass another to forge a wrong-audience token).
 */
