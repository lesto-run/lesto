 ...
 *
 * One composable `lesto()` app exposes a generic CRUD admin over the
 * `@lesto/admin` service as real HTTP routes:
 *
 ...
 *
 * `@lesto/admin` is a programmatic CRUD backbone, not an HTTP surface — it hands
 * you `list / get / create / update / destroy` over a `@lesto/db` Table and
 ...
 * malformed. Guards the seam to `@lesto/admin`: a bad `?limit=abc` or `/products/x`
...
/**
 * The routes, closing over the `@lesto/admin` service they front. Every handler
 ...
 */
export function buildAdminApp(deps: { admin: Admin; db: Db }): Lesto {
  const { admin, db } = deps;
 ...
 *
 * `@lesto/admin` invokes this *after* each committed write with a structured
 ...
 * the kernel, build the `@lesto/admin` service with the audit hook wired in, and
...
// one place — the example's own surface, never `@lesto/admin`'s internals.
