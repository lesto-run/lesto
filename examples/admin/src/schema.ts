/**
 * The two tables this example stands up, as `@keel/db` schema values, plus the
 * migrations the kernel runs on boot and the helpers `run.ts` / the test call.
 *
 * `products` is the resource the admin panel manages — the table `@keel/admin`
 ...
 *
 * `auditLog` is where the `onMutation` hook lands. `@keel/admin` does not own a
 ...
 *
 * Both tables are plain `@keel/db` schema values (no `extends Model`), migrated
...
export const SEED_PRODUCTS: readonly {
  ...
  { name: "Keel Tee", price: 2500, stock: 120, cost: 800 },
...
/** Insert the seed catalog directly through `@keel/db` (bypassing the admin's hook). */
