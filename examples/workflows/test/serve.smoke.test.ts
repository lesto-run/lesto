/**
 * The BEHAVIORAL half of `serve.ts`: does the workflows app actually BOOT and
 * SERVE over a real socket? The other tests drive the app in-process
 * (`app.handle`) — none proves the `serve.ts` entry spawns a `node:http` server
 * that answers a real `fetch()`. This does.
 *
 ...
 *
 * The GET /checkout/:orderId/trace read is chosen deliberately: it is a safe
 * side-effect-free route (no charge, no step run), so the smoke test proves the
 * server boots and routes WITHOUT tripping the demo's one-shot receipt fault or
 * its real (capped) settlement sleep. An order with no journal yet traces `[]`.
 *
...
describe("@lesto/workflows example — serve.ts boots over real HTTP", () => {
  it("serves GET /checkout/:orderId/trace over a real socket and exits 0 on SIGTERM", async () => {
    ...
    // A never-run order has an empty step journal, so its trace is `[]`. A real
    // HTTP round-trip, not the in-process `handle` the other tests use.
    const res = await fetch(`${base}/checkout/smoke-order/trace`);
