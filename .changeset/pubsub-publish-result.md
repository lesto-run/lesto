...
- The in-repo consumers (`packages/realtime/src/bus.ts` and `examples/reactive/src/app.ts`) already `void` the return value, so this is not an in-repo behavior change — but it is a public breaking API change for any external consumer reading the old bare count.

Bump level: `minor` — the repo is pre-1.0 (0.x) and treats minor as the breaking-change channel under lockstep versioning (`major` stays held for the eventual 1.0 launch). This is the queued breaking change that consumes the 0.2.0 slot the earlier `edge-pbkdf2-iteration-cap` changeset noted was reserved.
